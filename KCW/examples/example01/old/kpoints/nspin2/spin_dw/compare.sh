...
## CHECK PWSCF
a=`grep ! Si.scf.out | awk '{print $5}'`
a_ref=`grep ! reference/Si.scf.ref | awk '{print $5}'`
...
check=1
a=`grep high Si.nscf.out | awk '{print $7}'`
a_ref=`grep high reference/Si.nscf.ref | awk '{print $7}'`
err=`echo $a $a_ref | awk '{printf "%20.15f \n", sqrt(($1-$2)*($1-$2))}'`
if (( $(echo "$err > $tol_eig" |bc -l) )); then
 echo -e "${RED}  PWNSCF WARNING: ${NC} VBM does not match: $a $a_ref $err"
 check=0
fi
a=`grep high Si.nscf.out | awk '{print $8}'`
...
##   OCC
for i in `seq 2 5`; do
    a=`grep -A 4 "Final State" Si.wout | head -$i | tail -1|  awk '{print $11}'`
a_ref=`grep -A 4 "Final State" reference/Si.wref | head -$i | tail -1 | awk '{print $11}'`
...
##   EMP
for i in `seq 2 5`; do
    a=`grep -A 4 "Final State" Si_emp.wout | head -$i | tail -1|  awk '{print $11}'`
a_ref=`grep -A 4 "Final State" reference/Si_emp.wref | head -$i | tail -1 | awk '{print $11}'`
...
check=1
for i in `seq 1 8`; do
    a=`grep relaxed Si.kcw-screen.out | head -$i | tail -1|  awk '{print $12}'`
a_ref=`grep relaxed reference/Si.kcw-screen.ref | head -$i | tail -1 | awk '{print $12}'`
err=`echo $i $a $a_ref | awk '{printf "%20.15f \n", sqrt(($2-$3)*($2-$3))}'`
...
for i in `seq 1 8`; do 
 grep "KI  " Si.kcw-ham.out | head -$i | tail -1 > pp
 grep "KI  " reference/Si.kcw-ham.ref | head -$i | tail -1 >> pp
 for j in `seq 1 8`; do 
  col=`echo $j+1 | bc`
