...
cp -arv user_guide QE-logo.jpg qe-input-ref.html ${MINGW_BUILD_DIR}/qe-docs/
cp release-notes ${MINGW_BUILD_DIR}/qe-docs/Release-Notes.txt
popd
cp License ${MINGW_BUILD_DIR}/qe-docs/License.txt
cp README ${MINGW_BUILD_DIR}/qe-docs/README.txt

find ${MINGW_BUILD_DIR}/qe-docs/ -type f -name \*.txt  -print | xargs unix2dos
find ${MINGW_BUILD_DIR}/qe-docs/ -type f -name \*.html -print | xargs unix2dos
find ${MINGW_BUILD_DIR}/qe-docs/ -type f -name \*.css  -print | xargs unix2dos

make distclean

# serial 32-bit
mingw32-configure LIBDIRS=$PWD/install/blas-win32
make MANUAL_DFLAGS="-D_WIN32" all
make MANUAL_DFLAGS="-D_WIN32" xspectra
...
make MANUAL_DFLAGS="-D_WIN32 -I$PWD/install/mpich2-win32/include" all
make MANUAL_DFLAGS="-D_WIN32 -I$PWD/install/mpich2-win32/include" xspectra
STRIP=$(eval `rpm --eval %{mingw32_env}`; echo $STRIP)
...
make MANUAL_DFLAGS="-D_WIN32 -I$PWD/install/mpich2-win64/include" all
make MANUAL_DFLAGS="-D_WIN32 -I$PWD/install/mpich2-win64/include" xspectra
STRIP=$(eval `rpm --eval %{mingw64_env}`; echo $STRIP)
