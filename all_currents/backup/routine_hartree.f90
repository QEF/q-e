...
!
!do ibnd=1,nbnd
!   evp(1:npw,ibnd)=(evc_uno(1:npw,ibnd)-evc_due(1:npw,ibnd))/delta_t
!end do
...
!     end if
!     call mp_sum(s)
!     call dgemm ('N','T',nbnd,nbnd,nbnd,1.d0,s,nbnd,s,nbnd,0.d0,ss,nbnd)
!     evp=0.d0
!     do ibnd=1,nbnd
!        do jbnd=1,nbnd	  
!           do ig=1,npw
!              evp(ig,ibnd)=evp(ig,ibnd)+evc_uno(ig,jbnd)*s(ibnd,jbnd)
!        	     evp(ig,ibnd)=evp(ig,ibnd)-evc_due(ig,jbnd)*ss(ibnd,jbnd)
!           end do
!        end do
!     end do
!     evp(:,:)=evp(:,:)/delta_t
...
!do ibnd=1,nbnd
!   do ig=1,npw
!      evp(ig,ibnd)=evc_uno(ig,ibnd) * (0.d0,-1.d0) * tpiba * &
!      ( g(1,ig)*0.01 + g(2,ig)*0.0 + g(3,ig)*0.0  )  
!   end do
!end do
...
!                 write(iun,'("TEMP",f14.10)') 2.d0*ecurrent(ipol)
                 close(iun)
              end if
...
!   iun=find_free_unit()
!   open (iun, file = trim(tmp_dir)//trim(file_output),position='append')
!!   write(iun,'(3f17.13)') J_xc(1:3)
!   write(iun,'(3f17.13)') J_hartree(1:3)
!   write(iun,'(3f17.13)') J_kohn(1:3)
!   close(iun)
!end if
