...
!
  INTERFACE
    subroutine diropn_due(pref, unit, extension, recl, exst) 
       implicit none
...
file_output, file_dativel,blocco,passo,calcolato, prefix_thermal, outdir
...
!  call start_all_currents()
  ...
  CALL mp_startup ( )
#endif
  CALL environment_start ( 'HARTREE' )
  !
...
!------------------------------------------------------------------------
  ...
  CALL mp_bcast( prefix_thermal, ionode_id, MPI_COMM_WORLD )
  ...
  CALL mp_bcast( blocco,      ionode_id, MPI_COMM_WORLD )
  CALL mp_bcast( passo,       ionode_id, MPI_COMM_WORLD )
  CALL mp_bcast( calcolato,   ionode_id, MPI_COMM_WORLD )
  !
  prefix = trim(prefix_uno)
  call read_file
  dirname = TRIM(tmp_dir) //TRIM(prefix_due) //'.save'//'/'
  iunwfc  = 1000+iunwfc
  call diropn_due(trim(prefix_due), iunwfc, 'wfc', 2 * nwordwfc, exst ) 
