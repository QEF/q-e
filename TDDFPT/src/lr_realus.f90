...
MODULE lr_realus
  ...
  INTEGER,  ALLOCATABLE :: box(:,:), maxbox(:) ! box: the box index, maxbox: number of grid points 
  REAL(DP), ALLOCATABLE :: qsave(:) !q interpolated in these boxes
  REAL(DP), ALLOCATABLE :: boxrad(:) ! The radii of the boxes where q is nonzero
  REAL(DP), ALLOCATABLE :: boxdist(:,:), xyz(:,:,:)  !distance from centre and coordinates of grid points
  REAL(DP), ALLOCATABLE :: spher(:,:,:) !spherical harmonics calculated for each box
...
! Delete Delete Delete Delete
    ...
    ! 
   !------------------------------------------------------------------------
    SUBROUTINE betapointlist_v2()
      !------------------------------------------------------------------------
     !
     ! experimental replacement of betapointlist. 
     ! I am trying to counteract the fact that the beta functions are not given as expected
     ! in some UPFs.  
     ! 161208: I will try something derived on the Init_US2 
      USE constants,  ONLY : pi, eps8, eps16
      USE ions_base,  ONLY : nat, nsp, ityp, tau
      USE cell_base,  ONLY : at, bg, omega, alat
      USE gsmooth,    ONLY : nr1s, nr2s, nr3s, nrx1s, nrx2s, nrx3s, nrxxs, nls, nlsm
      USE uspp,       ONLY : okvan, indv, nhtol, nhtolm, ap,vkb, nkb
      USE uspp_param, ONLY : upf, lmaxq, nh, nhm
      USE atom,       ONLY : rgrid
      !USE pffts,      ONLY : npps
      USE fft_base,   ONLY : dffts
      ...
      betasave = 0.D0
      CALL stop_clock( 'realus:boxes' )
      ! Lets try back fourier transforming the vkb, maybe it works better in this case. 
      call init_us_2(npw_k(1),igk_k(:,1),xk(1,1),vkb)
        jh = 0
        do nt = 1, ntyp
           do ia = 1, nat
              if (ityp (ia) .eq.nt) then
                 do ih = 1, nh (nt)
                    jh = jh + 1
                    psic(1:nrxxs) = ( 0.D0, 0.D0 )
                    psic(nls(igk_k(:,1)))=vkb(:,jh)
                    psic(nlsm(igk_k(:,1)))=CONJG(vkb(:,jh))
                    call cft3 (psic, nr1s, nr2s, nr3s, nrx1s, nrx2s, nrx3s, 1)
                    betasave(ia,ih,1:mbia)=psic(box_beta(1:mbia,ia))
                enddo
              endif
          enddo
        enddo
      ...
      !
! Delete Delete Delete Delete
!          print *,"Dumping Betasave v2"
!          DO ia = 1, nat           
!           nt = ityp(ia)
!           print *,"atom ",ia," has ", nh (nt), " beta functions"
!           DO ih = 1, nh (nt)
!           write(tmp,fmt='("at",i1,"beta",i1)'),ia,ih
!           filename="rsv2-" // trim(tmp) // ".dump"
!           OPEN(UNIT=47,FILE=filename,STATUS='NEW',ACCESS = 'SEQUENTIAL')
!             DO ir = 1, nrxxs
!              !
!               ! ... three dimensional indexes
...
!
! Delete Delete Delete Delete      
    END SUBROUTINE betapointlist_v2
...
print *, "newd_r"
      ...
      !print *, "aux allocate"
      ALLOCATE( aux( nrxx ) )
      !if (.not. allocated(aux)) CALL errore( 'newd_r', 'not enough memory', 1 ) 
print *, "----------------------------2------------------------------"
      DO is = 1, nspin0
         !
...
print *, "----------------------------3------------------------------"
      ...
      !print *, "deeq calculated in real space"
      !
      RETURN
      !
      CONTAINS
