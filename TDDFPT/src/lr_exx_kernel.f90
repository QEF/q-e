...
!
SUBROUTINE lr_exx_apply_revc_int(psi, ibnd, nbnd, kpoint)
  !------------------------------------------------------------------
  !
  ...
  INTEGER, INTENT(IN) :: ibnd, nbnd, kpoint
  ...
  ELSE
     !
     psi(:) = psi(:) + revc_int_c(:,ibnd,kpoint)
     !
  ENDIF
...
SUBROUTINE lr_exx_revc0_init(orbital, kpoint)
  !------------------------------------------------------------------
  !
  ...
  !
  !  For the kpoint version also it performs the necessary rotations to
  ...
  ELSE
     ...
     DO ibnd=1,nbnd
        ...
        DO ikq=1,nkqs
           !
           IF (index_xk(ikq) .NE. kpoint) CYCLE
           !
           ! Now rotate k to q
