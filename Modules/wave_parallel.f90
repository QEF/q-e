...
MODULE wave_parallel
   USE kinds, ONLY: DP

   contains

   subroutine compute_dot_evc_parallel(t_minus, t_zero, t_plus, dot_evc, nbnd, npw, npwx, gstart)
      ...
      !! on output you have dot_evc*dt (the derivative multiplied by dt)
      use mp, only: mp_sum
...
! computes phi_v^c_punto using <s,s>
      ...
      do ibnd = 1, nbnd
         do jbnd = 1, nbnd
            ...
            dot_evc(ig, ibnd) = dot_evc(ig, ibnd) - t_zero(ig, jbnd)*ssa(ibnd, jbnd)
            dot_evc(ig, ibnd) = dot_evc(ig, ibnd) - t_minus(ig, jbnd)*sb(jbnd, ibnd)
            dot_evc(ig, ibnd) = dot_evc(ig, ibnd) + t_zero(ig, jbnd)*ssb(ibnd, jbnd)
