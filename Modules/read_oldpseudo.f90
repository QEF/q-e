...
!
      ...
      use uspp_param, only: qfunc, qfcoef, qqq, betar, dion, vloc_at, &
           rinner, kkbeta, lll, nbeta, nqf, nqlc, tvanp, iver, psd, oldvan
      use funct, only: set_dft_from_name, dft_is_hybrid, dft_is_meta
      use atom, only: zmesh, nchi, chi, lchi, r, rab, mesh, nlcc, &
           rho_at, rho_atc
...
!
!     nnlz, wwnl, ee  contain info (not used) on pseudo eigenstates
!
      read( iunps, '(i5,2f15.9)', err=100, iostat=ios ) &
           ( nnlz(iv),  wwnl(iv), ee(iv), iv=1,nchi(is) )
...
!
      ...
      use atom, only: rho_atc, r, rab, mesh, nlcc, lchi, chi, nchi, nchix, &
           xmin, zmesh, dx, oc, rho_at
      use uspp_param, only: nqlc, qfunc, vloc_at, rinner, psd, &
