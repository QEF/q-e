...
!=========================================================================
!
MODULE metagga_gpu                       !<GPU:metagga_gpu=>metagga_gpu>
!
USE exch_lda_gpu,  ONLY : slater_d   !<GPU:slater_d=>slater_d,exch_lda_gpu=>exch_lda_gpu>
USE corr_lda_gpu,  ONLY : pw_d, pw_spin_d   !<GPU:pw_d=>pw_d,pw_spin_d=>pw_spin_d,corr_lda_gpu=>corr_lda_gpu>
USE corr_gga_gpu,  ONLY : pbec_d, pbec_spin_d   !<GPU:pbec_d=>pbec_d,pbec_spin_d=>pbec_spin_d,corr_gga_gpu=>corr_gga_gpu>
!
 CONTAINS
!                             TPSS
!
...
attributes(DEVICE) &
#endif
SUBROUTINE tpsscxc_d (rho,grho,tau,sx,sc,v1x,v2x,v3x,v1c,v2c,v3c)
  ...
  !
  ! exchange
  CALL metax_d( rho, grho, tau, sx, v1x, v2x, v3x )                   !<GPU:metax_d=>metax_d>
  ! correlation
  CALL metac_d( rho, grho, tau, sc, v1c, v2c, v3c )                   !<GPU:metac_d=>metac_d>
  !
  RETURN
  !
END SUBROUTINE tpsscxc_d
!
!
!-------------------------------------------------------------------------
#if defined(__CUDA)
attributes(DEVICE) &
#endif
SUBROUTINE metax_d (rho,grho2,tau,ex,v1x,v2x,v3x)
  ...
  !
  rs = pi34/rho**third
  CALL slater_d( rs, ex_unif, vx_unif )                                  !<GPU:slater_d=>slater_d>
  CALL metaFX_d( rho, grho2, tau, fx, f1x, f2x, f3x )                    !<GPU:metaFX_d=>metaFX_d>
  !
  ex = rho*ex_unif
...
attributes(DEVICE) &
#endif
SUBROUTINE metac_d (rho,grho2,tau,ec,v1c,v2c,v3c)
  ...
  IF (rhoup > small) THEN
     !
     rs = (pi34/rhoup)**third
     CALL pw_spin_d( rs, 1.0_DP, ec_unif, vc_unif_s(1), vc_unif_s(2) )                                  !<GPU:pw_spin_d=>pw_spin_d>
     !
     IF (ABS(grhoup) > small) THEN
        ! zeta=1.0_DP-small to avoid pow_e of 0 in pbec_spin
        CALL pbec_spin_d( rhoup, 1.0_DP-small, grhoup**2, 1, &           !<GPU:pbec_spin_d=>pbec_spin_d>
...
attributes(DEVICE) &
#endif
SUBROUTINE metaFX_d (rho,grho2,tau,fx,f1x,f2x,f3x)
...
attributes(DEVICE) &
#endif
SUBROUTINE tpsscx_spin_d (rhoup, &
rhodw,grhoup2,grhodw2,tauup,taudw,sx,v1xup,v1xdw,v2xup,v2xdw,v3xup,v3xdw)
  ...
  IF (rhoup>small .AND. SQRT(ABS(grhoup2))>small &
     ...
     CALL metax_d( 2.0_DP*rhoup, 4.0_DP*grhoup2, &              !<GPU:metax_d=>metax_d>
  ...
  IF (rhodw > small .AND. SQRT(ABS(grhodw2)) > small &
     ...
     CALL metax_d( 2.0_DP*rhodw, 4.0_DP*grhodw2, &              !<GPU:metax_d=>metax_d>
...
attributes(DEVICE) &
#endif
SUBROUTINE tpsscc_spin_d (rho, &
zeta,grhoup,grhodw,atau,sc,v1cup,v1cdw,v2cup,v2cdw,v3cup,v3cdw)
  ...
  IF (rho <= small .OR. ABS(zeta) > 1.0_DP .OR. SQRT(ABS(grho)) <= small &
     ...
     v3c = 0.0_DP
  ELSE
     CALL metac_spin_d( rho, zeta, grhoup, grhodw, &              !<GPU:metac_spin_d=>metac_spin_d>
...
attributes(DEVICE) &
#endif
SUBROUTINE metac_spin_d (rho,zeta,grhoup,grhodw,tau,sc,v1up,v1dw,v2up,v2dw,v3)
  ...
  IF (rho > small) THEN
     ...
     v2_tmp = 0.0_DP
     rs = (pi34/rho)**third
     CALL pw_spin_d( rs, zeta, ec_u, vc_u(1), vc_u(2) )             !<GPU:pw_spin_d=>pw_spin_d>
     !
     IF ((ABS(grho) > small) .AND. (zeta <= 1.0_DP)) THEN
        CALL pbec_spin_d( rho, zeta, grho2, 1, ec_pbe, v1_pbe(1), v1_pbe(2), v2_tmp )             !<GPU:pbec_spin_d=>pbec_spin_d>
        v1up_pbe=v1_pbe(1)
        v1dw_pbe=v1_pbe(2)
     ELSE
        ec_pbe   = 0.0_DP
  ...
  IF (rhoup > small) THEN
     ...
     !
     rs = (pi34/rhoup)**third
     CALL pw_spin_d( rs, 1.0_DP, ec_u, vc_u(1), vc_u(2) )     !<GPU:pw_spin_d=>pw_spin_d>
     !
     IF (SQRT(grhoup2) > small) THEN
        CALL pbec_spin_d( rhoup, 1.0_DP-small, grhoup2, 1, &    !<GPU:pbec_spin_d=>pbec_spin_d>
  ...
  IF (rhodw > small) THEN
     ...
     IF (SQRT(grhodw2) > small) THEN
        !
        CALL pbec_spin_d( rhodw, -1.0_DP+small, grhodw2, 1, &   !<GPU:pbec_spin_d=>pbec_spin_d>
...
attributes(DEVICE) &
#endif
SUBROUTINE m06lxc_d (rho,grho2,tau,ex,ec,v1x,v2x,v3x,v1c,v2c,v3c)
  ...
  taub = taua               ! Tau is defined as summ_i( |nabla phi_i|**2 )
                              ! in the M06L routine
  !
  CALL m06lx_d( rhoa, grho2a, taua, ex, v1x, v2x, v3x )   !<GPU:m06lx_d=>m06lx_d>
  !
  ex = two * ex  ! Add the two components up + dw
  !
  v2x = 0.5_dp * v2x
  !
  CALL m06lc_d( rhoa, rhob, grho2a, grho2b, taua, taub, ec, v1c, v2c, v3c, &   !<GPU:m06lc_d=>m06lc_d>
              v1cb, v2cb, v3cb )
  !
  v2c = 0.5_dp * v2c
  !
END SUBROUTINE m06lxc_d
!
!
!-------------------------------------------------------------------------
#if defined(__CUDA)
attributes(DEVICE) &
#endif
SUBROUTINE m06lxc_spin_d (rhoup,rhodw,grhoup2,grhodw2, &
tauup,taudw,ex,ec,v1xup,v1xdw,v2xup,v2xdw,v3xup,v3xdw,v1cup,v1cdw,v2cup,v2cdw,v3cup,v3cdw)
  ...
  !
  CALL m06lx_d( rhoup, grhoup2, taua, exup, v1xup, v2xup, v3xup )   !<GPU:m06lx_d=>m06lx_d>
  CALL m06lx_d( rhodw, grhodw2, taub, exdw, v1xdw, v2xdw, v3xdw )   !<GPU:m06lx_d=>m06lx_d>
  !
  ex = exup + exdw
  !
  CALL m06lc_d( rhoup, rhodw, grhoup2, grhodw2, taua, taub, &       !<GPU:m06lc_d=>m06lc_d>
              ec, v1cup, v2cup, v3cup, v1cdw, v2cdw, v3cdw )
  !
END SUBROUTINE m06lxc_spin_d
! !
!
!===============================  M06L exchange ==========================
!
...
attributes(DEVICE) &
#endif
SUBROUTINE m06lx_d (rho,grho2,tau,ex,v1x,v2x,v3x)
  ...
  !
  IF (gh >= small) THEN
    CALL gvt4_d( xs2, zs, d0, d1, d2, d3, d4, d5, alpha, hg, dhg_dxs2, dhg_dzs )   !<GPU:gvt4_d=>gvt4_d>
  ELSE
    hg = zero
    dhg_dxs2 = zero
    dhg_dzs  = zero
  ...
  !
  CALL pbex_m06l_d( two*rho, four*grho2, sx_pbe, v1x_pbe, v2x_pbe )   !<GPU:pbex_m06l_d=>pbex_m06l_d>
  !
...
attributes(DEVICE) &
#endif
SUBROUTINE pbex_m06l_d (rho,grho2,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE m06lc_d (rhoa,rhob, &
grho2a,grho2b,taua,taub,ec,v1c_up,v2c_up,v3c_up,v1c_dw,v2c_dw,v3c_dw)
  ...
  ELSE
    ...
    rs   = rsa
    zeta = one
    CALL pw_spin_d( rs, zeta, ec_pw_a, vc_v(1), vc_v(2) )    !<GPU:pw_spin_d=>pw_spin_d>
    vc_pw_a = vc_v(1)
    vv      = vc_v(2)
    !
    CALL gvt4_d( xs2a, zsa, ds0, ds1, ds2, ds3, ds4, ds5, alpha_s, hga, dhga_dxs2a, dhga_dzsa )   !<GPU:gvt4_d=>gvt4_d>
    CALL gfunc_d( cs, gama_s, xs2a, gsa, dgsa_dxs2a )   !<GPU:gfunc_d=>gfunc_d>
    !
  ...
  ELSE
    ...
    zeta = one
    rs   = rsb
    CALL pw_spin_d( rs, zeta, ec_pw_b, vc_v(1), vc_v(2) )   !<GPU:pw_spin_d=>pw_spin_d>
    vc_pw_b = vc_v(1)
    vv      = vc_v(2)
    !
    CALL gvt4_d( xs2b, zsb, ds0, ds1, ds2, ds3, ds4, ds5, alpha_s, hgb, dhgb_dxs2b, dhgb_dzsb )   !<GPU:gvt4_d=>gvt4_d>
    CALL gfunc_d( cs, gama_s, xs2b, gsb, dgsb_dxs2b )   !<GPU:gfunc_d=>gfunc_d>
    !
  ...
  ELSE
    ...
    rs    = (pi34/rho)**f13
    !
    CALL gvt4_d( xs2ab, zsab, dab0, dab1, dab2, dab3, dab4, dab5, alpha_ab, hgab, &   !<GPU:gvt4_d=>gvt4_d>
               dhgab_dxs2ab, dhgab_dzsab )
    !
    CALL pw_spin_d( rs, zeta, ec_pw_ab, vc_v(1), vc_v(2) )   !<GPU:pw_spin_d=>pw_spin_d>
    vc_pw_up=vc_v(1) ; vc_pw_dw=vc_v(2)
    !
    CALL gfunc_d( cab, gama_ab, xs2ab, gsab, dgsab_dxs2ab )   !<GPU:gfunc_d=>gfunc_d>
    !
...
attributes(DEVICE) &
#endif
SUBROUTINE gfunc_d (cspin,gama,xspin,gs,dgs_dx)
...
attributes(DEVICE) &
#endif
SUBROUTINE gvt4_d (x,z,a,b,c,d,e,f,alpha,hg,dh_dx,dh_dz)
