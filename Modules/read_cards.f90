...
!      mass(i)   ( real )              atomic mass ( in u.m.a, carbon mass is 12.0 )
...
! ----------------------------------------------------------------
      ...
      SUBROUTINE card_atomic_positions( input_line, prog )
        ...
        END IF
        IF( .NOT. taspc ) THEN
          CALL errore(' card_atomic_positions  ', ' ATOMIC_SPECIES must be present before ', 2 )
        END IF
        IF( ntyp > nsx ) THEN
        ...
        DO ia = 1, nat
          ...
          ELSE IF( nfield == 7 ) THEN
            READ(input_line,*) lb_pos, rd_pos(1,ia), rd_pos(2,ia), rd_pos(3,ia), &
                                    if_pos(1,ia), if_pos(2,ia), if_pos(3,ia)
          ELSE
            CALL errore( ' iosys ', ' wrong number of columns  in ATOMIC_POSITIONS ', sp_pos(ia) )
          END IF
          ...
          IF( sp_pos(ia) < 1 .OR. sp_pos(ia) > ntyp ) THEN
            CALL errore( ' iosys ', ' wrong index in ATOMIC_POSITIONS ', ia )
          END IF
...
! ----------------------------------------------------------------
     ...
     SUBROUTINE card_vhmean( input_line )
        ...
        tvhmean_inp = .true.
        CALL read_line( input_line )
        READ (input_line,*) vhiunit_inp, vhnr_inp, vhrmin_inp, vhrmax_inp, vhasse_inp
        tread = .TRUE.
        RETURN
     END SUBROUTINE
...
! ----------------------------------------------------------------
     ...
     SUBROUTINE card_optical( input_line )
        ...
        IF( empty_states_nbnd < 1 ) THEN
          CALL errore(' card_optical ', ' empty states are not computed ', 2 )
        END IF
        toptical_card = .TRUE.
        CALL read_line( input_line )
...
! ----------------------------------------------------------------
      ...
      SUBROUTINE card_ion_velocities( input_line )
        ...
        IF( .NOT. taspc ) THEN
          CALL errore(' card_ion_velocities ', ' ATOMIC_SPECIES must be present before ', 2 )
        END IF
        ...
        IF( ion_velocities == 'from_input' ) THEN
          ...
          DO ia = 1, nat
            ...
            ELSE
              CALL errore(' iosys ', ' wrong entries in ION_VELOCITIES ', ia)
            END IF
...
!      CONSTR_TOL  tolerance for keeping the constraints satisfied
!      CONSTR_TYPE(.) CONSTR(1,.) CONSTR(2,.)  type of constrain and atoms indeces
!                          object of the constraint. I.E.: 1 ia1 ia2
!                          "1" is the constrain type (fixed distance)
!                          "ia1 ia2" are the indeces of the atoms (as they
!                          appear in the 'POSITION' CARD) whose distance
...
! ----------------------------------------------------------------
     ...
     SUBROUTINE card_constraints( input_line )
        ...
        DO i = 1, nconstr_inp
          ...
          CASE (2)
            READ(input_line,*) constr_type_inp(i), constr_inp(1,i), constr_inp(2,i), &
                         constr_dist_inp(i)
          CASE DEFAULT
            READ(input_line,*) constr_type_inp(i), constr_inp(1,i), constr_inp(2,i)
          END SELECT
