!
MODULE exch_gga_gpu !<GPU:exch_gga_gpu=>exch_gga_gpu>
!
CONTAINS
!
!-----------------------------------------------------------------------
...
attributes(DEVICE) &
#endif
SUBROUTINE becke88_d (rho,grho,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE ggax_d (rho,grho,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE pbex_d (rho,grho,iflag,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE hcth_d (rho,grho,sx,v1x,v2x)
  ...
  CALL pwcorr_d( ra, cg1, g, dg )                           !<GPU:pwcorr_d=>pwcorr_d>
  era1 = g
  dera1_dra = dg
  CALL pwcorr_d( rab, cg0, g, dg )                          !<GPU:pwcorr_d=>pwcorr_d>
  erab0 = g
  derab0_drab = dg
  ...
  RETURN
  !
END SUBROUTINE hcth_d
    !
    !-------------------------------------------------------
#if defined(__CUDA)
attributes(DEVICE) &
#endif
SUBROUTINE pwcorr_d (r,c,g,dg)
...
attributes(DEVICE) &
#endif
SUBROUTINE optx_d (rho,grho,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE wcx_d (rho,grho,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE pbexsr_d (rho,grho,sxsr,v1xsr,v2xsr,omega)
...
REAL(DP), VALUE, intent(IN) :: omega
  ...
  ENDIF
  !
  CALL wpbe_analy_erfc_approx_grad_d( rho, s, omega, fx, d1x, d2x ) !<GPU:wpbe_analy_erfc_approx_grad_d=>wpbe_analy_erfc_approx_grad_d>
  !
  sxsr  = ex*fx                        ! - ex
...
attributes(DEVICE) &
#endif
SUBROUTINE rPW86_d (rho,grho,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE c09x_d (rho,grho,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE sogga_d (rho,grho,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE pbexgau_d (rho,grho,sxsr,v1xsr,v2xsr,alpha_gau)
  ...
  IMPLICIT NONE
  !
REAL(DP), VALUE, intent(IN) :: alpha_gau
  REAL(DP), INTENT(IN) :: rho, grho
  REAL(DP), INTENT(OUT) :: sxsr, v1xsr, v2xsr
  !
  ! ... local variables
  ...
  IF (s > 10.D0) THEN
     s = 10.D0
  ENDIF
  CALL pbe_gauscheme_d( rho, s, alpha_gau, fx, d1x, d2x )   !<GPU:pbe_gauscheme_d=>pbe_gauscheme_d>
  sxsr = ex*fx                        ! - EX
  dsdn = -4.D0/3.D0*s/rho
  v1xsr = vx*fx + (dsdn*d2x+d1x)*ex   ! - VX
  dsdg = us*rr
  ...
  RETURN
  !
END SUBROUTINE pbexgau_d
    !
    !-----------------------------------------------------------------------
#if defined(__CUDA)
attributes(DEVICE) &
#endif
SUBROUTINE pbe_gauscheme_d (rho,s,alpha_gau,Fx,dFxdr,dFxds)
       ...
       IF (ABS(One/bx/bx) < 1.0D-4) THEN
          cx = TayExp_d(-One/bx/bx)                               !<GPU:TayExp_d=>TayExp_d>
       ELSE
          cx = EXP(-One/bx/bx) - One
       ENDIF
       !
       Nx = bx * Prefac * ( SQRT(PI) * qe_erf_d(One/bx) + &       !<GPU:qe_erf_d=>qe_erf_d>
       ...
       RETURN
       !
END SUBROUTINE pbe_gauscheme_d
!
!
!-------------------------------------------------
#if defined(__CUDA)
...
FUNCTION TayExp_d (x)
  ...
  IMPLICIT NONE
  REAL(DP), INTENT(IN) :: X
  REAL(DP) :: TAYEXP_d                        !<GPU:TAYEXP_d=>TAYEXP_d>
  INTEGER :: NTERM,I
  ...
  DO 10 I = 2, NTERM
     ...
     SUMVAL = SUMVAL + IVAL
10     CONTINUE
  TAYEXP_d = SUMVAL                      !<GPU:TAYEXP_d=>TAYEXP_d>
  !
  RETURN
  !
END FUNCTION TayExp_d
!
!
!
!-------------------------------------------------------------------------
...
attributes(DEVICE) &
#endif
SUBROUTINE PW86_d (rho,grho,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE becke86b_d (rho,grho,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE b86b_d (rho,grho,iflag,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE cx13_d (rho,grho,sx,v1x,v2x)
...
attributes(DEVICE) &
#endif
SUBROUTINE becke88_spin_d (rho_up,rho_dw,grho_up,grho_dw,sx_up,sx_dw,v1x_up,v1x_dw,v2x_up,v2x_dw)
...
attributes(DEVICE) &
#endif
SUBROUTINE wpbe_analy_erfc_approx_grad_d (rho,s,omega,Fx_wpbe,d1rfx,d1sfx)
        ...
        G_a    = srpi * (Fifteen*E + Six*C*(One+F*s2)*DHs + &
                      ...
                      * (One / (Sixteen * DHs72))           &
                       - f34*pi*SQRT(A) * EXP(f94*H*s2/A) * &
                         (One - qe_erf_d(f32*s*SQRT(H/A)))                    !<GPU:qe_erf_d=>qe_erf_d>
        !
        d1sG_a = (One/r32)*srpi *                           &
                 ((r36*(Two*H + d1sH*s) / (A12*SQRT(H/A)))  &
                  ...
                  - ((r54*EXP(f94*H*s2/A)*srpi*s*(Two*H+d1sH*s)*     &
                     qe_erfc_d(f32*SQRT(H/A)*s))                       &      !<GPU:qe_erfc_d=>qe_erfc_d>
                     / A12))
        !
      ...
      IF (HsbwA94 < expfcutoff) THEN
        !
        piexperf = pi*EXP(HsbwA94)*qe_erfc_d(HsbwA9412)                   !<GPU:qe_erfc_d=>qe_erfc_d>
        ! expei    = Exp(HsbwA94)*Ei(-HsbwA94)
        expei    = EXP(HsbwA94)*(-expint_d(1,HsbwA94))                   !<GPU:expint_d=>expint_d>
...
END SUBROUTINE wpbe_analy_erfc_approx_grad_d
!
!---------------------------------------------------------------------
#if defined(__CUDA)
attributes(DEVICE) &
...
FUNCTION qe_erf_d (x)
  ...
  REAL(DP) :: qe_erf_d    !<GPU:qe_erf_d=>qe_erf_d>
  ...
  if (abs (x) > 6.0d0) then
     ...
     !
     qe_erf_d = sign (1.0d0, x)                                                  !<GPU:qe_erf_d=>qe_erf_d>
  else
     if (abs (x)  <= 0.47d0) then
        x2 = x**2
        qe_erf_d=x *(p1 (1) + x2 * (p1 (2) + x2 * (p1 (3) + x2 * p1 (4) ) ) ) &  !<GPU:qe_erf_d=>qe_erf_d>
                / (q1 (1) + x2 * (q1 (2) + x2 * (q1 (3) + x2 * q1 (4) ) ) )
     else
        qe_erf_d = 1.0d0 - qe_erfc_d(x)                                            !<GPU:qe_erf_d=>qe_erf_d,qe_erfc_d=>qe_erfc_d>
     endif
  endif
  !
  return
...
FUNCTION qe_erfc_d (x)
  ...
  !
  REAL(DP),intent(in) :: x
  REAL(DP)            :: qe_erfc_d                                         !<GPU:qe_erfc_d=>qe_erfc_d>
  ...
  if (ax > 26.0d0) then
     ...
     !
     qe_erfc_d = 0.0d0                                                            !<GPU:qe_erfc_d=>qe_erfc_d>
  elseif (ax > 4.0d0) then
     x2 = x**2
     xm2 = (1.0d0 / ax) **2
     qe_erfc_d = (1.0d0 / ax) * exp ( - x2) * (pim1 + xm2 * (p3 (1) &             !<GPU:qe_erfc_d=>qe_erfc_d>
          + xm2 * (p3 (2) + xm2 * (p3 (3) + xm2 * (p3 (4) + xm2 * p3 (5) &
          ) ) ) ) / (q3 (1) + xm2 * (q3 (2) + xm2 * (q3 (3) + xm2 * &
          (q3 (4) + xm2 * q3 (5) ) ) ) ) )
  elseif (ax > 0.47d0) then
     x2 = x**2
     qe_erfc_d = exp ( - x2) * (p2 (1) + ax * (p2 (2) + ax * (p2 (3) &            !<GPU:qe_erfc_d=>qe_erfc_d>
          + ax * (p2 (4) + ax * (p2 (5) + ax * (p2 (6) + ax * (p2 (7) &
          + ax * p2 (8) ) ) ) ) ) ) ) / (q2 (1) + ax * (q2 (2) + ax * &
          (q2 (3) + ax * (q2 (4) + ax * (q2 (5) + ax * (q2 (6) + ax * &
          (q2 (7) + ax * q2 (8) ) ) ) ) ) ) )
  else
     qe_erfc_d = 1.0d0 - qe_erf_d(ax)                          !<GPU:qe_erfc_d=>qe_erfc_d, qe_erf_d=>qe_erf_d>
  endif
  !
  ! erf(-x)=-erf(x)  =>  erfc(-x) = 2-erfc(x)
  !
  if (x < 0.0d0) qe_erfc_d = 2.0d0 - qe_erfc_d                                      !<GPU:qe_erfc_d=>qe_erfc_d>
  !
  return
...
!
      ...
      INTEGER, INTENT(IN) :: n
      REAL(DP), INTENT(IN) :: x
      REAL(DP) :: expint_d                             !<GPU:expint_d=>expint_d>
...
!     EPS=1E-9, FPMIN=1E-30
      ...
      IF (n == 0) THEN
         expint_d = exp(-x)/x                                             !<GPU:expint_d=>expint_d>
         RETURN
      END IF
      nm1 = n-1
      IF (x == 0.0d0) THEN
         expint_d = 1.0d0/nm1                                             !<GPU:expint_d=>expint_d>
      ELSE IF (x > 1.0d0) THEN
         ...
         expint_d = h*EXP(-x)                                             !<GPU:expint_d=>expint_d>
      ELSE
         IF (nm1 /= 0) THEN
            expint_d = 1.0d0/nm1                                          !<GPU:expint_d=>expint_d>
         ELSE
            expint_d = -LOG(x)-euler                                      !<GPU:expint_d=>expint_d>
         END IF
         fact = 1.0d0
         do i=1,maxit
            fact = -fact*x/i
...
!               del = fact*(-LOG(x)-euler+sum(1.0d0/arth(1,1,nm1)))
            END IF
            expint_d = expint_d + del                                       !<GPU:expint_d=>expint_d>
            IF (ABS(del) < ABS(expint_d)*eps) EXIT                        !<GPU:expint_d=>expint_d>
         END DO
         IF (i > maxit) STOP !CALL errore('expint','series failed',1)
      END IF
END FUNCTION EXPINT_d
!
END MODULE
