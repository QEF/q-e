...
CONTAINS
  ...
  SUBROUTINE card_xml_cell ( )
    ...
    IF ( ierr /= 0 ) CALL errore( 'read_xml_pw', 'error scanning begin of CELL &
         &card', ABS( ierr ) )
    !
    IF ( found ) THEN
       !
  ...
  SUBROUTINE card_xml_atomic_species( )
    ...
    IF ( found ) THEN
       ...
       DO is = 1, ntyp
          !
          CALL iotk_scan_dat(xmlinputunit, 'SPECIE', psfile, attr = attr2, ierr = ierr )
          IF ( ierr /= 0 ) CALL errore( 'card_xml_atomic_species',  'error &
               &reading SPECIE node', abs( ierr ) )
          !
          ...
          IF ( atom_mass(is) <= 0.0_DP ) THEN
             CALL errore( ' card_xml_atomic_species ', ' invalid  atom mass',  is )
          ENDIF
          !
          DO ip = 1, is - 1
             !
       ...
       IF (ierr/=0)  CALL errore( 'card_xml_atomic_species', 'error scanning end of ATOMIC_SPECIES &
            &node', ABS( ierr ) )
       !
    ELSE
       !
  ...
  !
  !
  ! ... Subroutine that reads a single image inside ATOMIC_LIST node
  !
  SUBROUTINE read_image( image, r_image )
    !
    IMPLICIT NONE
    !
    INTEGER, INTENT( in ) :: image
    REAL( DP ), INTENT( out ), DIMENSION( 3, nat ) :: r_image
    !
    !
    ...
    DO
       ...
       !
       CALL iotk_scan_dat( xmlinputunit,'POSITION', r_image( 1:3, ia ), attr = attr, ierr = ierr )
  ...
  !                                                                         !
  !    <CONST type="constr_type(1)" target="CONSTR_TARGET(1)">              !
  !        <REAL rank="1" n1="4">                                           !
  ...
  SUBROUTINE card_xml_constraints( )
    ...
    DO i = 1, nconstr_inp
       ...
       IF ( ierr /= 0 ) CALL errore( 'card_xml_constraints', &
            'error scanning begin of CONST node', abs( ierr ) )
       !
       CALL iotk_scan_attr( attr2, 'type', constr_type_inp(i), ierr = ierr )
       IF ( ierr /= 0 ) CALL errore( 'card_xml_constraints', &
            'error reading type attribute of CONST node', abs( ierr ) )
       !
       CALL iotk_scan_attr( attr2, 'target', constr_target_inp(i), found = found, ierr = ierr )
       IF ( ierr /= 0 ) CALL errore( 'card_xml_constraints', &
            'error reading target attribute of CONST node', abs( ierr ) )
       !
       ...
       END SELECT
       !
       CALL iotk_scan_end( xmlinputunit, 'CONSTSTRAINT', ierr = ierr )
       IF ( ierr /= 0 ) CALL errore( 'card_xml_constraints', &
            'error scanning end of CONST node', abs( ierr ) )
       !
    ENDDO
    !
    RETURN
    !
    !
10  CALL errore( 'card_xml_constraints', 'error reading data inside CONST node', i )
    !
    !
  ...
  SUBROUTINE card_xml_climbing_images( )
    ...
    IF ( CI_scheme == 'manual' ) THEN
       ...
       !
       CALL iotk_scan_begin( xmlinputunit, 'INTEGER', attr2, ierr = ierr )
       IF ( ierr /= 0 ) CALL errore( 'card_xml_climbing_images', 'error scanning begin of &
       ...
       IF ( ierr /= 0 ) CALL errore( 'card_xml_climbing_images', 'error scanning end of &
            &INTEGER node', abs( ierr ) )
       !
       CALL iotk_scan_attr( attr2, 'n1', num_climb_images, ierr = ierr )
