...
!
!-----------------------------------------------------------------------
SUBROUTINE perdew86_spin( rho, zeta, grho, sc, v1c, v2c )
  !---------------------------------------------------------------------
  !! Perdew gradient correction on correlation: PRB 33, 8822 (1986)
  !! spin-polarized case.
  !
  ...
  !  
  v1c(1) = sc * ( (1._DP + phi) * dcn / cn - ( (4._DP / 3._DP) - &    
       (7._DP/6._DP)*phi)/rho) - sc * ddd / dd * (1._DP - zeta)/rho  
  v1c(2) = sc * ( (1._DP + phi) * dcn / cn - ( (4._DP / 3._DP) - &    
       (7._DP/6._DP)*phi)/rho) + sc * ddd / dd * (1._DP + zeta)/rho
...
!
!-----------------------------------------------------------------------
SUBROUTINE ggac_spin( rho, zeta, grho, sc, v1c, v2c )
  !---------------------------------------------------------------------
  ...
  !  
  sc     = rho * (h0 + h1)  
  v1c(1) = h0 + h1 + dh0up + dh1 + dh0zup + dh1zup  
  v1c(2) = h0 + h1 + dh0up + dh1 + dh0zdw + dh1zdw  
  v2c    = ddh0 + ddh1  
  !
  !
  RETURN
...
!
!-------------------------------------------------------------------
SUBROUTINE pbec_spin( rho, zeta, grho, iflag, sc, v1c, v2c )
  !-----------------------------------------------------------------
  ...
  !  
  sc     = rho * h0  
  v1c(1) = h0 + dh0up + dh0zup  
  v1c(2) = h0 + dh0dw + dh0zdw  
  v2c    = ddh0  
  !
  !
  RETURN
...
!
!------------------------------------------------------------------------
SUBROUTINE lsd_glyp( rho_in, grho, grho_ud, sc, v1c, v2c, v2c_ud )
  !----------------------------------------------------------------------
  !! Lee, Yang, Parr: gradient correction part.
  !
  USE kinds, ONLY: DP
  ...
  v1c(1) = dlaaa*grhoaa + dlaba*grhoab + dlbba*grhobb      
  v1c(2) = dlaab*grhoaa + dlabb*grhoab + dlbbb*grhobb      
  v2c(1) = 2._DP*dlaa      
  v2c(2) = 2._DP*dlbb      
  v2c_ud = dlab  
  !    
