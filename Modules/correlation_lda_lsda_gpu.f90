...
MODULE corr_lda_gpu !<GPU:corr_lda_gpu=>corr_lda_gpu>
...
attributes(DEVICE) &
#endif
SUBROUTINE pz_d (rs,iflag,ec,vc)
...
attributes(DEVICE) &
#endif
SUBROUTINE pzKZK_d (rs,ec,vc,vol)
...
attributes(DEVICE) &
#endif
SUBROUTINE vwn1_rpa_d (rs,ec,vc)
...
attributes(DEVICE) &
#endif
SUBROUTINE pw_d (rs,iflag,ec,vc)
...
attributes(DEVICE) &
#endif
SUBROUTINE pz_polarized_d (rs,ec,vc)
...
attributes(DEVICE) &
#endif
SUBROUTINE pz_spin_d (rs,zeta,ec,vc_up,vc_dw)
  ...
  CALL pz_d( rs, 1, ecu, vcu )                                        !<GPU:pz_d=>pz_d>
  !
  ! polarization contribution
  CALL pz_polarized_d( rs, ecp, vcp )                                 !<GPU:pz_polarized_d=>pz_polarized_d>
  !
  ...
  RETURN
  !
END SUBROUTINE pz_spin_d
!
!-------------------------------------------------------------------------------
#if defined(__CUDA)
attributes(DEVICE) &
#endif
SUBROUTINE vwn_spin_d (rs,zeta,ec,vc_up,vc_dw)
   ...
   !
   CALL padefit_ParSet1_d( sqrtrs, 1, ecP, vcP )              ! ecF = e_c Paramagnetic     !<GPU:padefit_ParSet1_d=>padefit_ParSet1_d>
   CALL padefit_ParSet1_d( sqrtrs, 2, ecF, vcF )              ! ecP = e_c Ferromagnetic    !<GPU:padefit_ParSet1_d=>padefit_ParSet1_d>
   CALL padefit_ParSet1_d( sqrtrs, 3, ac,  dac )              ! ac = "spin stiffness"      !<GPU:padefit_ParSet1_d=>padefit_ParSet1_d>
   !
   ac = ac * iddfz0
   dac = dac * iddfz0
...
attributes(DEVICE) &
#endif
SUBROUTINE padefit_ParSet1_d (x,i,fit,dfit)
   ...
   !
END SUBROUTINE padefit_ParSet1_d
#if defined(__CUDA)
attributes(DEVICE) &
#endif
SUBROUTINE padefit_ParSet2_d (x,i,fit,dfit)
   ...
   dfit = fit - A(i) / 3.0_DP + A(i)*x/6.0_DP * (  txbfx + 4.0_DP*b(i)*itxbQ + &
           bx0fx0(i) * ( 2.0_DP/xx0 - txbfx - 4.0_DP*(b(i)+2.0_DP*x0(i))*itxbQ )  )
   !
END SUBROUTINE padefit_ParSet2_d
!
!
!-----------------------------------------------------------------------------------------
#if defined(__CUDA)
attributes(DEVICE) &
#endif
SUBROUTINE vwn1_rpa_spin_d (rs,zeta,ec,vc_up,vc_dw)
   ...
   dfz = cfz2 * (trup13 - trdw13)                       ! df / dzeta
   !
   CALL padefit_ParSet2_d( sqrtrs, 1, ecP, vcP ) ! ecF = e_c Paramagnetic    !<GPU:padefit_ParSet2_d=>padefit_ParSet2_d>
   CALL padefit_ParSet2_d( sqrtrs, 2, ecF, vcF ) ! ecP = e_c Ferromagnetic   !<GPU:padefit_ParSet2_d=>padefit_ParSet2_d>
   CALL padefit_ParSet2_d( sqrtrs, 3, ac,  dac ) ! ac = "spin stiffness"     !<GPU:padefit_ParSet2_d=>padefit_ParSet2_d>
   !
   ac = ac * iddfz0
   dac = dac * iddfz0
   De = ecF - ecP - ac                                  ! e_c[F]-e_c[P]-alpha_c/(ddf/ddz(z=0))
   ...
   vc_dw = dec1 - (1.0_DP + zeta)*dec2                  !          +[sign(s)-zeta]*(de_c/dzeta)
   !
END SUBROUTINE vwn1_rpa_spin_d
!
!
!-----------------------------------------------------------------------
#if defined(__CUDA)
attributes(DEVICE) &
#endif
SUBROUTINE pw_spin_d (rs,zeta,ec,vc_up,vc_dw)
...
attributes(DEVICE) &
#endif
SUBROUTINE lsd_lyp_d (rho,zeta,elyp,vlyp_up,vlyp_dw)
