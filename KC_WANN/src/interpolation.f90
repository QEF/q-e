...
MODULE interpolation
  ...
  USE control_kc_wann,      ONLY : num_wann, Hamlt, Hamlt_R
  !
  !
  IMPLICIT NONE
  !
...
CONTAINS
  ...
  SUBROUTINE FT_ham(ham, h_dim, ham_t, ir, k_to_R)
    ...
    USE control_kc_wann,      ONLY : spin_component, irvect, xk_bands, centers, &
                                     use_ws_distance
  ...
  SUBROUTINE read_wannier_centers( )
    ...
    USE control_kc_wann,      ONLY : seedname, have_empty, num_wann_occ, num_wann_emp, &
...
20  CALL errore ( 'read_wannier_centers', 'error while reading', 1 ) 
  ...
  SUBROUTINE print_bands_to_file( eigvl )
    ...
    USE control_kc_wann,      ONLY : xk_bands, nks_bands, num_wann
    ...
    IF ( ionode ) THEN
      ...
      !
      !
      OPEN( 100, file=trim(prefix)//'.kc_bands.dat', status='unknown' )
      !
      DO iband = 1, num_wann
        !
        DO ik = 1, nks_bands
  ...
  SUBROUTINE dealloc_interpolation()
    ...
    !
    USE control_kc_wann,      ONLY : xk_bands, wk_bands, centers
    !
    !
    IMPLICIT NONE
    !
