...
subroutine read_wannier ()
  ...
  !
  USE control_kc_wann,      ONLY : l_unique_manifold
  !
  IMPLICIT NONE 
  !
...
END subroutine read_wannier
  ...
  !
  USE kinds,                ONLY : DP
  USE control_kc_wann,      ONLY : unimatrx, seedname, has_disentangle, &
                                   unimatrx_opt, num_wann, kc_iverbosity
  ...
  if ( kc_iverbosity .gt. 1) WRITE(stdout,'(/,5X, "INFO: Optimal Matrix READ")') 
  !
  IF (ionode) THEN   
    !
    DO ik = 1, nkstot_eff
  ...
  CLOSE (1002)
  !
  if (kc_iverbosity .gt. 1) WRITE(stdout,'(/,5X, "INFO: total number of Wannier functions", i5)') num_wann
  !
#if defined (DEBUG)
  ! WRITE
  DO ik =1, nkstot_eff
...
END subroutine read_wannier_unique_manifold 
  ...
  !
  USE kinds,                ONLY : DP
  USE control_kc_wann,      ONLY : unimatrx, have_empty, num_wann_occ, seedname, &
  ...
  if (.NOT. have_empty .AND. kc_iverbosity .gt. 1) WRITE(stdout,'(/,5X, "INFO: total number of Wannier functions", i5)') num_wann
  !
