...
!-----------------------------------------------------------------------
subroutine kc_setup_ham
  !-----------------------------------------------------------------------
  !
  !! As kc_setup.f90 plus hamiltonian specific setups
  !
  !
  USE kinds,             ONLY : DP
...
!  USE xml_io_base,     ONLY : write_rho
  ...
  IF (.NOT.exst.AND..NOT.exst_mem) THEN
     CALL errore ('kc_setup_ham', 'file '//trim(prefix)//'.wfc not found', 1)
  END IF
