...
SUBROUTINE bare_pot ( rhor, rhog, vh_rhog, delta_vr, delta_vg, iq, delta_vr_, delta_vg_ )
  ...
  USE control_kc_wann,      ONLY : spin_component, kc_iverbosity
  USE gvect,                ONLY : g
  USE qpoint,               ONLY : xq
  ...
  ! ... Gygi-Baldereschi sheme, see setup_coulomb_exx) 
  !
  DO ig = 1, ngms
    !
  ...
  IF (do_comp_mt .AND. lgamma) then
     ...
     ! ... (do_compt_mt = .false if more than one k-point (see kc_readin.f90) 
     !
     IF (kc_iverbosity .gt. 1 ) WRITE(stdout,'(5x, " ADDING Martyna-Tuckerman correction" ,/)')
     !
     ALLOCATE( vaux( ngms ) )
