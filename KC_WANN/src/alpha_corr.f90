...
SUBROUTINE alpha_corr ( iwann, delta)
  ...
  !!            the 2nd order approx to the Hamiltonian in kc_ham.x we do not add delta_u
  !!            to the denominator so that the product of the bare kc hamiltonian at 2nd
  ...
  USE control_kc_wann,      ONLY : num_wann_occ, nqstot
  ...
  IF ( dft_is_gradient ( ) .AND. nqstot .gt. 1 ) &
  ...
  CONTAINS 
    ...
    SUBROUTINE xc_energy_n ( iwann, en, eig, krnl )
      ...
      USE control_kc_wann,       ONLY : evc0, iuwfc_wann, num_wann, spin_component, nqstot, iurho_wann
      USE wvfct,                 ONLY : npwx
      ...
      DO ik = 1, nks
        ...
        !eig_k = eig_k/( dffts%nr1*dffts%nr2*dffts%nr3 )*omega
        eig_k = eig_k/( dffts%nr1*dffts%nr2*dffts%nr3 )
        CALL mp_sum (eig_k, intra_bgrp_comm) 
!        WRITE(stdout, '("Nic ik, Eig_k =", i5, f15.9)') ik, eig_k
        eig = eig + eig_k
        !
      ENDDO
...
!        WRITE(stdout, '("Nic ik, krnl_q =", i5, f15.9)') ik, krnl_q
