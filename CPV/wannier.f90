...
12133   FORMAT(' LARGEST TIMESTEP IN THE SD / CG DIRECTION FOR SPREAD OPTIMIZATION = ',f10.7)
    ...
    IF(wf_efield) THEN
       ...
       WRITE( stdout, * )
       WRITE( stdout, 12121) efx0
       WRITE( stdout, 12122) efy0
       WRITE( stdout, 12123) efz0
       WRITE( stdout, 12128) efx1
       WRITE( stdout, 12129) efy1
       WRITE( stdout, 12130) efz1
       IF(wf_switch) THEN
           WRITE( stdout, 12127) sw_len
       END IF
       WRITE( stdout, * )
...
SUBROUTINE ef_potential( nfi, rhos, bec, deeq, betae, c0, cm, emadt2, emaver, verl1, verl2, c2, c3 )
  ...
  IF(wf_efield) THEN
    IF(field_tune) THEN
      ...
      efy=e_tuned(2)
      efz=e_tuned(3)
      WRITE( stdout, 12131) efx, efy,efz
12131      FORMAT(' wf_efield Now ' ,3(f12.8,1x))
    ELSE
      IF(wf_switch) THEN
        par=0.d0
...
!=============================================================
  ...
  IF(calwf.EQ.4) THEN
    ...
    WRITE( stdout, * ) 'Wannier Functions Written to unit',ndw
    CALL mp_end()
