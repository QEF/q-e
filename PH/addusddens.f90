...
#include "machine.h"
  ...
  if (.not.lgamma) call mallocate(qpg, 3  , ngm)  
  if (iflag.eq.0) call mallocate(drhous, nrxx, nspin)  
  ...
  enddo
  if (iflag.eq.0) call mfree (drhous)  
  if (.not.lgamma) call mfree (qpg)  
  call mfree (ylmk0)  
  call mfree (qmod)  
  call mfree (qg)  
  call mfree (sk)  
