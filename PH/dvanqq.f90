...
#include "machine.h"
  ...
  real(kind=DP), pointer :: qmod (:), qmodg (:), qpg (:,:), &
       ylmkq (:,:), ylmk0 (:,:)
  ! the modulus of q+G
  ! the modulus of G
  ! the  q+G vectors
  ...
  complex(kind=DP), pointer :: sk (:), aux1 (:), aux2 (:),&
       aux3 (:), aux5 (:,:,:), veff (:,:), qgmq (:) 
  ! auxiliary variables
  ...
  call mallocate(aux2,  ngm)  
  call mallocate(aux3,  ngm)  
  call mallocate(aux5,  ngm ,nat,  3 )  
  call mallocate(qmodg, ngm)  
  call mallocate(veff , nrxx , nspin)  
  call mallocate(ylmk0, ngm , lqx * lqx)  
  if (.not.lgamma) then  
     call mallocate(ylmkq,ngm , lqx * lqx)  
     call mallocate(qpg ,3, ngm)  
     call mallocate(qmod, ngm)  
     call mallocate(qgmq, ngm)  
  else  
     qgmq =>qgm  
  endif
  !
