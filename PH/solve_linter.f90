...
#include "machine.h"
  ...
  complex(kind=DP),pointer :: ldos (:,:), ldoss (:,:),&
       dvscfin(:,:,:), drhoscfh (:,:,:), dvscfout (:,:,:),&
       dvscfins (:,:,:), dbecsum (:,:,:,:), spsi (:), auxg (:),&
       aux1 (:), ps (:)
  ...
  call mallocate(dvscfin , nrxx , nspin , npe)  
  if (doublegrid) then  
     call mallocate(dvscfins , nrxxs , nspin , npe)  
  else  
     dvscfins => dvscfin  
  endif
  call mallocate(drhoscfh , nrxx , nspin , npe)  
  call mallocate(dvscfout , nrxx , nspin , npe)  
  call mallocate(auxg ,npwx)  
  call mallocate(dbecsum , (nhm * (nhm + 1))/2 , nat , nspin , npe)
  ...
  if (lmetq0) then  
     call mallocate(ldos , nrxx   , nspin)  
     call mallocate(ldoss,  nrxxs , nspin)  
...
155 continue  
  ...
  if (lmetq0) call mfree (ldoss)  
  if (lmetq0) call mfree (ldos)  
  call mfree (eprec)  
  call mfree (h_diag)  
  call mfree (spsi)  
  call mfree (aux1)  
