...
subroutine plot_2d (nx, ny, m1, m2, x0, e, ngm, g, vgc, alat, &
  ...
  complex(kind=DP), pointer :: eigx (:), eigy (:), carica(:,:)  

  call mallocate(eigx,  nx)  
  call mallocate(eigy,  ny)  
  call mallocate(carica, nx , ny)  
...
subroutine plot_2ds (nx, ny, x0, ngm, g, vgc, output_format, &
  ...
  parameter (pi = 3.14159265358979d0)  
  complex(kind=DP), pointer :: carica (:,:)
  complex(kind=DP) :: eig  

  call mallocate(carica, nx , ny)  
  CALL mallocate(r ,3, nx , ny)  
...
subroutine plot_3d (alat, at, nat, tau, atm, ityp, ngm, g, vgc, &
  ...
  complex(kind=DP), pointer :: eigx (:), eigy (:), eigz (:)
  real(kind=DP), pointer :: carica (:,:,:)

  call mallocate(eigx,  nx)  
  call mallocate(eigy,  ny)  
  call mallocate(eigz,  nz)  
  call mallocate(carica, nx , ny , nz)  
