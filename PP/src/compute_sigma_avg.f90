...
SUBROUTINE compute_sigma_avg(sigma_avg,becp_nc,ik,lsigma)
  ...
  DO ibnd = 1, nbnd
     ...
     IF (lsigma(1)) THEN
        ...
        ENDDO
        IF (doublegrid) CALL fft_interpolate_real( rho%of_r(1,2), dffts, rho%of_r(1,2), dfftp )
     ENDIF
     IF (lsigma(2)) THEN
        DO ir = 1,dffts%nnr
        ...
        ENDDO
        IF (doublegrid) CALL fft_interpolate_real( rho%of_r(1,3), dffts, rho%of_r(1,3), dfftp )
     ENDIF
     IF (lsigma(3)) THEN
        DO ir = 1,dffts%nnr
        ...
        ENDDO
        IF (doublegrid) CALL fft_interpolate_real( rho%of_r(1,4), dffts, rho%of_r(1,4), dfftp )
     ENDIF
