...
module wannier
   ...
   integer  :: iun_nnkp, iun_mmn, iun_amn, nnbx, n_wannier
...
! read wfc at k+b
         ...
         call gk_sort (xk(1,ikp), ngm, g, ecutwfc / tpiba2, npw, igkq, g2kin)
! compute the phase
         phase(:) = (0.d0,0.d0)
...
! loops on bands
         do ibnd=1,nbnd
            ...
            do jbnd=1,nbnd   
               ...
               call reduce(mmn,2)
               write (iun_mmn,'(7i6)') ibnd, jbnd, ik, ikp, &
...
subroutine compute_amn
   ...
   open (unit=iun_amn, file='wannier.mmn',form='formatted')
   ...
   do ik=1,nkstot
      call davcio (evc, nwordwfc, iunwfc, ik, -1 )
      call gk_sort (xk(1,ik), ngm, g, ecutwfc / tpiba2, npw, igk, g2kin)
      call generate_guiding_functions
      do ibnd=1,nbnd
         do iw=1,n_wannier   
            amn = ZDOTC(npw,evc(1,ibnd),1,gf(1,iw),1)
            call reduce(amn,2)
            write(iun_amn,*) ibnd,iw,ik, amn
         end do
      end do
   end do
   close (iun_amn)
...
!
subroutine generate_guiding_functions
   ...
   do iw =1, n_wannier
      do ig=1,npw
         ...
         gf(ig,iw) = exp(-alpha_w(iw)*g2kin(ig)) * &
