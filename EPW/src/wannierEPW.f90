...
MODULE  wannierEPW
   ...
   COMPLEX(KIND = DP), ALLOCATABLE :: gf(:, :)  ! guding_function(npwx,n_proj)
   ...
   REAL(KIND = DP), ALLOCATABLE :: center_w(:, :)   ! projection function center (crystal coords.) center_w(3,n_wannier)
   INTEGER,  ALLOCATABLE :: spin_eig(:)     ! "1" or "-1" to denote projection onto up or down spin states
   REAL(KIND = DP), ALLOCATABLE :: spin_qaxis(:, :) ! spin quantisation axis (Cartesian coords.)
   ...
   REAL(KIND = DP), ALLOCATABLE :: zaxis(:, :) ! defines the axis from which the polar angle \theta in spherical coords. i
                                       ! is measured. zaxis(3,n_wannier)
   REAL(KIND = DP), ALLOCATABLE :: xaxis(:, :) ! defines the axis from which the azimuthal angle \phi in spherical coords.
                                       ! is measured. xaxis(3,n_wannier)
   REAL(KIND = DP), ALLOCATABLE :: alpha_w(:) ! alpha_w(n_wannier) ( called proj_zona in wannier spec)
   !
   REAL(KIND = DP), ALLOCATABLE :: csph(:, :)  ! expansion coefficients of gf on QE ylm function (16,n_wannier)
   ...
   REAL(KIND = DP)              :: rlatt(3,3)     ! real lattices (Cartesian coords., units of Angstrom)
   REAL(KIND = DP)              :: glatt(3,3)     ! recip. lattices (Cartesian coords., units of reciproval Angstrom)
   REAL(KIND = DP), ALLOCATABLE :: kpt_latt(:, :)  ! k-points in crystal coords. kpt_latt(3,iknum)
   REAL(KIND = DP), ALLOCATABLE :: atcart(:, :)    ! atom centres (Cartesian coords., units of Angstrom). atcart(3,nat)
   ...
   COMPLEX(KIND = DP), ALLOCATABLE :: m_mat(:, :, :, :) ! overlap matrices between neighbouring periodic parts of Bloch
                                              ! eigenstates at each k-point M_mn(k,b)=<u_mk|u_nk+b>
                                              ! m_mat(num_bands,num_bands,nnb,iknum)
   COMPLEX(KIND = DP), ALLOCATABLE :: a_mat(:, :, :)   ! matrices describing the projection of n_wannier trial orbitals
   ...
   REAL(KIND = DP), ALLOCATABLE    :: wann_spreads(:)   ! spread of WFs in Angstrom^2. wann_spreads(n_wannier)
   REAL(KIND = DP)                 :: spreads(3)        ! values of \Omega, \Omega_I, and \tilde{\Omega}
   REAL(KIND = DP), ALLOCATABLE    :: eigval(:, :)       ! eigenvalues \epsilon_nk corresponding to the \psi_nk eigenstates
