...
\subsection{Who should read (and who should ''write'') this guide}
...
\item if you get conflicts, or if the updated code doesn't work any longer, you haven't yet won. Look into the conflicting section: in most cases conflicts are trivial (format changes, white spaces) or easily solved (the part of the code you were modifying has been moved to another place, for instance). Sometimes, somebody else has done changes that are incompatible with yours during the same period. Look into the ChangeLog (use the script <nowiki>cvs2cl.pl</nowiki> to get an updated ChangeLog) to understand what may have happened.  Use <nowiki>cvs update -D 2006-oct-12</nowiki> to update to the CVS version of 12 oct 2006: this may be useful if you suspect that the incompatibile change happened after that date. CVS versions can be ''tagged''; tags are visible with <nowiki>cvs status -v</nowiki>. Use <nowiki>cvs update -r TAG</nowiki> to update to the CVS version with tag TAG. In all cases, use <nowiki>cvs update -A</nowiki> to revert to the last version.
\end{itemize}
Developers can be contacted at the mail address: '''developers''' ''at'' '''pwscf''' ''dot'' '''org'''.
...
\item by first pre-compiling with the C preprocessor <nowiki>cpp</nowiki>.
\end{itemize}

In the first case, one needs to specify in the <nowiki>make.sys</nowiki> file the fortran compiler option that tells the compiler to pre-process first. In the second case, one needs to
specify the C precompiler and options (if needed) in <nowiki>make.sys</nowiki>.
...
The code contains C-style preprocessing directives. Most fortran compilers directly support them; some don't, and preprocessing is ''hand-made'' by the makefile using the C preprocessor <nowiki>cpp</nowiki>. The C preprocessor may:
\begin{itemize}
...
''Fully Scalable'': Arrays that are distributed across processors of a pool.
...
''Partially Scalable'': Arrays that are distributed across processors of the
...
* Files ''mat\_z.1'', ''lambda.dat'' are unformatted files, containing a single record;
...
\item conversions should be explicitely indicated. For conversions to real (including taking the real part of a complex number), use DBLE.  For conversions to complex, use CMPLX. For the complex conjugate, use CONJG. For the imaginary part, use AIMAG.  Note that CMPLX is preprocessed by f\_defs.h and performs an explicit cast:
\begin{verbatim}
...
* add pippo to the input\_parameters.f90 file containing the namelist control
\begin{verbatim}
              INTEGER :: pippo = 0
...
* add pippo to the control\_default subroutine (contained in module read\_namelists.f90 )
\begin{verbatim}
...
* add pippo to the control\_bcast subroutine (contained in module read\_namelists.f90 )
 \begin{verbatim}
