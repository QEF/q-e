...
\begin{quote}
  ...
  \texttt{include/fft_defs.h}: contains the type of fortran variable
...
\texttt{make.sys} yourself, to run \texttt{makedeps.sh} to generate
\texttt{*/make.depend} files, and (if needed) to generate file
...
If \texttt{include/fft\_defs.h} is not created by configure,
copy and edit the template \texttt{include/fft\_defs.h.in}. 
...
Newer Mac OS-X machines with Intel CPUs and gcc4+g95 are supported
by \texttt{configure}. With the Intel compiler \texttt{ifort} and 
MKL libraries, use \texttt{install/Make.macos-intel} 
