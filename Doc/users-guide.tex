\documentclass[12pt,a4paper]{article}
 \def\version{CVS}
 \def\stableversion{2.1.5} % last stable release
...
\begin{itemize}
  ...
  \item atomic: a program for atomic calculations and generation of
        pseudopotentials.
  \item iotk: Input-Output toolkit,
\end{itemize}
...
\begin{itemize}
  ...
  \item constant-temperature simulation with Nos\`e thermostats
        (including Nose-Hoover chains for each atom)
...
The restart files of all the programs are compatible (if data are):
you can run FPMD with a restart file from CP, and vice versa. 
For CP, there are implementations of a dynamics for metals using 
...
for PWgui. Providing binaries would require too much effort
...
\end{verbatim}
    ...
    \texttt{INPUT\_FPMD} and \texttt{INPUT\_CP} in the \texttt{Doc/}
