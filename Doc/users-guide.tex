...
\end{verbatim}
and fails to run configure properly.

The easiest way to correct this bug is to help the XLF compiler to  
find the correct location of gcc. To do that, follow precisely the  
two commands:
\begin{enumerate}
\item {\tt sudo mv /etc/opt/ibmcmp/xlf/8.1/xlf.cfg \\
                   /etc/opt/ibmcmp/xlf/8.1/xlf.cfg.2006.MM.DD.HH.MM.SS} \\
with MM.DD.HH.MM.SS is the current date (MM=month, DD=day etc...)
\item {\tt
...
\end{enumerate}
it replaces the xlf.cfg with the correct location (info by Pascal 
Thibeadeau).

\paragraph{T3E}

T3D/T3E is no longer supported.
