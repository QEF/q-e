...
Deinzer, Stefano Fabris, Adriano Mosca Conte, Mickael Profeta, Carlo
Sbraccia (SISSA), Anton Kokalj (Jo\v{z}ef Stefan Institute,
Ljubljana).
...
follow the instructions in \texttt{README.configure} to generate a new
\texttt{configure} file.
Please report success to the developers.
If the nontrivial task of configuring \texttt{configure} scares you,
use instead:
...
did not in the past, or we don't know if they do) C-style
...
\texttt{Install/Make.*}, and remember that only FFTW version $<3$ is
supported: FFTW version 3 requires changes in the code.
...
where modules are with respect to version $< 7$.
...
presently not supported: use FFTW instead.
...
working directory, where \texttt{prefix} is a string that is prepended
to all file names (default value: \texttt{prefix='pwscf'}).
Note that just killing the process may left the output files in an
unusable state.
...
Error messages should be self-explanatory, but unfortunately this is
not always true.
...
\begin{itemize}
  ...
  \item
    IBM SP3: under some circumstances (typically a large number of
    k-points) we get an error in \texttt{cdiaghg} that is
    reproducible, but disappears if we change anything in the
    calculation.
    We don't know what happens and why.
    Try to use conjugate-gradient diagonalization
    (\texttt{diagonalization='cg'}).
  \item
...
\begin{itemize}
  ...
  \item
    ...
    vector.
    If the FFT grid dimension along that direction is not divisible
    respectively by 2 or by 3, the symmetry operation will not
    transform the FFT grid into itself.
    Such symmetry operations are disabled to prevent problems with
    symmetrization.
    See above for a discussion on manually setting FFT grid
    dimensions.
  ...
  \item
    ...
    operation of the system (meaning that the cell is actually a
    supercell).
    In this case, all symmetry operations containing fractional
    translations are disabled.
    The reason is that in this rather exotic case there is no (simple)
    way to select those symmetry operations forming a group (in the
