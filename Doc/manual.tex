...
Currently active developers include Gerardo Ballabio (CINECA), Gernot
Deinzer, Stefano Fabris, Adriano Mosca Conte, Carlo Sbraccia (SISSA,
Trieste), Anton Kokalj (Jo\v{z}ef Stefan Institute, Ljubljana).
...
All trademarks mentioned in this manual belong to their respective
owners.
...
of the standard BLAS and LAPACK.
Download the appropriate binaries from:
\htmladdnormallink{\texttt{http://www.netlib.org/}}%
                  {http://www.netlib.org/},
or download and compile the sources (this may take some time and
effort). 
Note that ATLAS is not a complete replacement for LAPACK: ATLAS
...
on Linux Alphas, check the following page:\break
\htmladdnormallink%
...
The ESPRESSO package uses a unified pseudopotential format (UPF)\break
...
Tests and examples can be downloaded from the Test and Examples Page\break
...
\end{verbatim}
    ...
    \texttt{IONIC\_VELOCITIES}\break card, with the list of velocities in
    atomic units.
