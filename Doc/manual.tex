...
full LAPACK replacement. Axel Kohlmeyer maintains a set of ATLAS libraries, 
containing all of LAPACK and no external reference to fortran libraries:\\
\htmladdnormallink%
{\texttt{http://www.theochem.rub.de/\~{}axel.kohlmeyer/cpmd-linux.html\#atlas}}%
...
to open some shared libraries, which will lead to all kinds of 
problems (i.e. crashes) if libc/libm/libpthreads are not linked 
dynamically. Machines using glibc-2.2.4 and older seem ok. So if
one wants to share precompiled binaries, they should be compiled 
on the oldest available machine. Crashes due to multithreading 
...
Also note that some combinations of not-so-recent versions of MKL 
