...
This manual covers the installation and usage of ESPRESSO (opEn-Source
Package for Research in Electronic Structure, Simulation, and
Optimization), version \version.

ESPRESSO is composed of the following packages:
\begin{description}
  \item [PWscf] (Plane-Wave Self-Consistent Field):
    a set of programs for electronic structure calculations within
    Density-Functional Theory and Density-Functional Perturbation
    Theory, using a Plane-Wave basis set and pseudopotentials.

  \item [FPMD] (First Principles Molecular Dynamics) and \textbf{CP}:
    two programs for Car-Parrinello \emph{ab initio} molecular
    dynamics, also with a Plane-Wave basis set and pseudopotentials.
    (It is our long-term goal to merge FPMD and CP into a single
    program.)
  ...
  \item [atomic]:
    a set of programs for atomic calculations and generation of
    pseudopotentials.
\end{description}
...
\begin{itemize}
  ...
  \item the Pw\_forum mailing list
        ...
        See the PWscf web site for instructions on how to register and
        how to search the archives of the mailing list.
...
Running ESPRESSO on MS-Windows is possible in principle, but not
supported: a \texttt{Makefile} is provided (from an old release of
PWscf), but has not been tested with the current version and may not
work.
...
Non-colinear magnetism and spin-orbit interactions are also
implemented, although at an experimental stage.
...
\begin{itemize}
  ...
  \item variable-cell dynamics (FPMD only (*** \`e vero? ***))
  \item spin-polarized and/or many-k-points calculation
  \item Nudged Elastic Band (NEB) for energy barriers and reaction
        paths
  \item *** altro? ***
\end{itemize}
CP works with both norm-conserving and Ultrasoft pseudopotentials,
while FPMD is currently limited to norm-conserving (*** \`e vero?
***).
...
The FPMD and CP packages are both derived from the original code by
Roberto Car and Michele Parrinello (then at SISSA).
FPMD has been subsequently developed by
*** Chiarotti? Scandolo? Focher? ***
(SISSA)
and Carlo Cavazzoni
\htmladdnormallink{(\texttt{c.cavazzoni@cineca.it})}%
                  {mailto:c.cavazzoni@cineca.it}
(SISSA, then CINECA),
while CP has been developed by *** ??? ***

PWgui was written by Anton Kokalj.
...
the CINECA National Supercomputing Center in Bologna
(\htmladdnormallink{\texttt{http://www.cineca.it/}}%
                   {http://www.cineca.it/}),
under the responsibility of Carlo Cavazzoni.
\hyphenation{ge-rar-do}
Currently active developers include
Gerardo Ballabio (CINECA), 
Gernot Deinzer, Stefano Fabris, Adriano Mosca Conte, Carlo Sbraccia
(SISSA), 
Anton Kokalj (Jo\v{z}ef Stefan Institute, Ljubljana).

Several other people, mostly but not exclusively former SISSA students, 
contribute or have contributed to PWscf. Among them we mention:
\begin{quote}
\hyphenation{laz-ze-ri}
Dario Alf\`e, Francesco Antoniella, Mauro Boero, Claudia Bungaro,
Paolo Cazzato, Gabriele Cipriani, Matteo Cococcioni, Alberto
Debernardi, Guido Fratesi, Ralph Gebauer, Michele Lazzeri, Kurt
M\"ader, Nicola Marzari, Francesco Mauri, Pasquale Pavone, Mickael
Profeta, Guido Roma, Kurt Stokbro, Pascal Thibaudeau, Renata Wentzcovitch.
\end{quote}
...
\begin{enumerate}
  \item download and unpack the ESPRESSO source distribution;
  \item configure ESPRESSO for your system, compilers and libraries;
...
To uncompress and unpack the source package, move it to an empty
directory of your choice, \texttt{cd} to that directory, and run the
command:
\medskip
...
An automatic configuration procedure, generated with the GNU Autoconf
package, is available since version 2.0.
From the top directory of the ESPRESSO source tree, just type:
\begin{verbatim}
  ./configure
\end{verbatim}
...
configuration, you may either edit the \texttt{make.sys} file, or try
to generate a new \texttt{configure} file following the instructions
in \texttt{README.configure} (please report success to the
developers).
If \texttt{configure} stops with an error and the nontrivial task of
configuring \texttt{configure} scares you, or if you don't know what
...
\noindent
where \emph{your-system} is one of the following:
\begin{quote}
  \texttt{alpha}:     HP-Compaq alpha workstations, HP-Compaq
                      compiler\\
  \texttt{alphaMPI}:  HP-Compaq alpha parallel machines, HP-Compaq\\
                      \strut\qquad compiler\\
...
minor changes are needed to port ESPRESSO to an unsupported machine.
...
You can select the appropriate preprocessing options in 
\texttt{make.sys}, knowing that:
\begin{itemize}
...
\end{itemize} 
if \texttt{configure} doesn't do it for you.
...
are using the manual configuration, bchoose the appropriate line
...
PWscf uses a unified pseudopotential format (UPF)\\
...
(\htmladdnormallink{\texttt{http://www.pwscf.org/pseudo.html}}%
...
Since ESPRESSO v.~2.1, a PP generation package is provided with
ESPRESSO, in the directory \texttt{atomic/} (sources) and
\texttt{atomic\_doc/} (documentation, tests and examples). 
...
\begin{itemize}
  ...
  \item
    Jos\'e-Lu\'\i{}s Martins' code (Norm-Conserving PPs):\\
    \htmladdnormallink%
...
typical sign of such behaviours is that the self-consistency error 
