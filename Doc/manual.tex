...
following codes for the calculation of electronic-structure 
properties within Density-Functional Theory, using a 
...
\begin{description}
  ...
  \item [atomic]
    a program for atomic calculations and generation of
    pseudopotentials.
\end{description}
Documentation, in addition to what provided in this manual,
can be found in:
\begin{itemize}
...
Stefano Baroni, Stefano de Gironcoli, Andrea Dal Corso (SISSA),
Paolo Giannozzi,
and others.
...
This manual was written (mostly) by Paolo Giannozzi, Gerardo Ballabio,
Carlo Cavazzoni.
...
All trademarks mentioned in this manual 
...
An automatic configuration script, generated with the GNU Autoconf
...
in \texttt{README.configure} (please report success to the developers). 
The latter option is only for experts.
If \texttt{configure} stops with an error, you typically have a 
misconfigured fortran compiler, or no compiler at all, or a machine 
that does not allow direct interactive execution of fortran compiled 
codes.
...
(\texttt{CPPFLAGS}) and/or compiler options (\texttt{F90FLAGS}).

\paragraph{Please Note:}

if you change \texttt{CPPFLAGS} or \texttt{F90FLAGS} after the first
...
ESPRESSO uses the well-known linear algebra BLAS (Basic Linear Algebra 
Subroutines) and LAPACK (Linear Algebra PACKage) libraries.
...
of plain BLAS/LAPACK. Download the appropriate binaries from:\\
\htmladdnormallink{\texttt{http://www.netlib.org/}}%
                  {http://www.netlib.org/},
or download and compile the sources (this  may take some time and effort). 
Note that ATLAS is not a complete replacement for LAPACK: ATLAS contains 
...
\htmladdnormallink{\texttt{http://www.cs.utexas.edu/users/flame/goto}}
...
In general, if you want to use external precompiled libraries, 
you need to correctly specify their location in the 
\texttt{make.sys} file.
...
in the \texttt{install/} directory. 
% Note that systems marked with (+) in the above list do not support, 
% or did not in the past, or we don't know if they do, C-style
% preprocessing directives in fortran compilers.
...
\begin{itemize}
  ...
  \item
    ...
    \texttt{FPMD/par2.x} and the postprocessing code 
    \texttt{FPMD/fpmdpp.x}.
  \item
