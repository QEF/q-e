...
Now, to install ESPRESSO, you must:
\begin{enumerate}
...
\begin{verbatim}
  ...
  make all
\end{verbatim}
A \texttt{bin/} directory will be generated, containing all the
compiled executable programs (actually, symlinks to them).
...
\texttt{configure} also recognizes the following command-line options:
\begin{quote}
...
\begin{itemize}
  ...
  \item MKL together with Intel compilers (shipped by Intel, free for
        non-commercial use)
  \item ATLAS (free software,
        \htmladdnormallink%
        {\texttt{http://math-atlas.sourceforge.net/}}%
        {http://math-atlas.sourceforge.net/})
  \item FFTW (free software,
        \htmladdnormallink{\texttt{http://www.fftw.org/}}%
        {http://www.fftw.org/})
\end{itemize}
and a few others.  Note that FFTW must be version 2; version 3 is not
yet supported.
...
\texttt{configure} attempts to find those libraries, but may fail if
they have been installed at non-standard places.  You should examine
the final value of \texttt{LIBS} (either in the output of
...
\begin{verbatim}
  ./configure LIBS="-L/usr/lib/math -lfftw -llapack -lblas"
\end{verbatim}
...
If you want to use the FFTW library, the \texttt{fftw.h} include file
is also required.  If the configuration script wasn't able to find it,
you can specify its location in the \texttt{INCLUDEFFTW} environment
variable.
For example:
\begin{verbatim}
...
file manually.  The \texttt{install/} directory contains several
examples (named \texttt{Make.*}): you may choose the one that looks
most appropriate for your machine, copy it to \texttt{make.sys}, and
tweak it until you obtain successful compilation.  If this ever
happens, please let us know, and we'll try to support your machine in
the next version of \texttt{configure}.

\paragraph{Please Note:}

If you change any compilation flags after a previous (successful or
failed) compilation, you must run \texttt{make clean} before
recompiling, unless you know exactly which routines are affected by
the changed options and how to force their recompilation.
