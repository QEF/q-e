...
To install ESPRESSO, you need working C and fortran-95 compilers 
...
directory of the ESPRESSO distribution.
Those examples try to exercise all the programs and features of the
ESPRESSO package: for details, see the \texttt{README} file in each
example's directory.
...
as needed:
\begin{quote}
  \texttt{BIN\_DIR=} directory where ESPRESSO executables reside\\
...
root of the ESPRESSO source tree.
...
don't use an NFS-mounted directory).
  ...
  \item
If you have compiled the parallel version of ESPRESSO (that is the
...
ESPRESSO. Most so-called ``fortran-90'' compilers implement the 
...
ESPRESSO compiles and works on all non-buggy, properly configured 
...
If ESPRESSO does not work for some reason on a PC cluster, try first 
...
A frequent problem is that ESPRESSO does not read from standard 
...
the examples distributed with ESPRESSO (see section \ref{runexamples},
``Run examples'').
...
by Anton Kokalj, included in the ESPRESSO distribution.
...
You may take the examples distributed with ESPRESSO as templates for
