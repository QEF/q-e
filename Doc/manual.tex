...
\begin{itemize}
  \item the \texttt{Doc/} directory of the PWscf distribution;
  ...
  \item the ``Scientific Software'' page of the Democritos web site:\\
        (\htmladdnormallink{\texttt{http://www.democritos.it/scientific.php}}%
...
Non-colinear magnetism and spin-orbit interactions are also implemented, 
although at an experimental stage.
...
Starting from version 2.0, a Graphical User Interface (GUI) is
included in the package.
The GUI may be used in producing input data files needed to run PWscf.
...
PWscf is free software, released under the GNU General Public License
(\htmladdnormallink{\texttt{http://www.pwscf.org/License.txt}}%
                   {http://www.pwscf.org/License.txt},
or the file \texttt{License} in the top directory of the PWscf
distribution).
...
Presently, the PWscf package is only distributed in source form; some
precompiled executables (binary files) are provided only for the GUI.
...
\begin{enumerate}
  \item download and unpack the PWscf source distribution;
  \item configure PWscf for your system, compilers and libraries;
  \item compile some or all of the executables in the package.
\end{enumerate}

\subsection{Download and unpack PWscf}

The latest stable release of the PWscf source package can be downloaded 
from the PWscf web site at this URL:
\smallskip
...
\noindent
The release corresponding to this manual is version \version. The
bravest may access the version under development via anonymous CVS: 
see file \texttt{README.cvs} contained in the distribution.
...
From the top directory of the PWscf source tree, just type:
\smallskip
...
\begin{itemize}
  ...
  \item
    \texttt{make tools} produces several utility programs, mostly for phonon
    calculations, in \texttt{pwtools/} (see list below).
  \item
...
\htmladdnormallink{\texttt{http://www.tcm.phy.cam.ac.uk/\~{}mdt26/casino.html}}%
...
typical sign of such behavious is that the self-consistency error 
