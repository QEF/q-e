...
\item Suggesting changes: contact developers submitting an ''Issue'' on
  GitLab:\\
		\texttt{GitLab.com/QEF/q-e/Issues}, or posting to the 
  developers mailing list:\\
		\texttt{developers@lists.quantum-espresso.org} mailing list.
...
first of all register on GitLab, following the instructions.

{\bf Important notice: qe-forge.org will be phased out end of February 2018.}

All \qe\ developer are {\em strongly} invited to subscribe to the
developers' mailing list,
\texttt{developers@lists.quantum-espresso.org}.
Subscribe using the links
...
{\bf Notice:} the development model of \qe\ has undergone
significant changes after release 6.2.1. The development has moved to 
GitLab. There is also a GitHub mirror, that you can access via e.g.:
\begin{verbatim}
   git clone https://GitLab.com/QEF/q-e.git
\end{verbatim}

This repository is "one-way", only for pull operation: it is automatically
kept aligned with the official GitLab repository. Commits in this GIT 
repository will be overwritten. See Sect.\ref{Sec:git}, ``Using git'', 
for more instructions on how to use \texttt{git}.\\
...
are ``tightly bound'' to \qe, that is, use \qe\ code and routines. Frequently 
update your copy (using command \texttt{git pull}), verify if changes made 
meanwhile by other developers conflict with your changes.
...
Two currently active test-farm can be accessed at 
\href{http://130.186.13.198:8010/#/}{CINECA}
