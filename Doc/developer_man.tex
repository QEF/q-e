...
instead of (or in addition to) this guide.
...
\subsection{\qe\ as a distribution}

\qe\ is not organized as a monolithic code, but rather as a
{\em distribution} (integrated suite) of ``packages'' that can 
be installed on demand and with varying degrees of integration. 
It is thus possible to contribute:
\begin{itemize}
...
field of atomistic simulations. \qeforge\ provides a 
repository, mailing lists, a wiki\footnote{Usage of the
wiki provided by \qeforge\ is currently disabled for 
security reasons.}, upload space, a bug tracking facility, 
various other tools that are useful for developers.
...
in an existing project: just obtain the permission from the 
leader of the project you are interested in.

You may need to register your SSH keys in order to have read-write 
access the repository (if you have such permissions). The procedure 
is as follows:
\begin{enumerate}
...
commit messages (it is allowed to send replies to commit 
messages in case of doubts or questions or remarks
over a specific commit).
\end{itemize}
\end{itemize}
...
selected cases, that's ok, as long as the code stops (or ate least,
...
\section{Stable vs development versions}
...
The logic goes nore or less as follows:
\begin{itemize}
...
{\bf spiegare qui il casinismo del file che punta ai tar.gz}
...
\label{SubSec:Inst}
In order to compile and link \qe, one has to run the \configure\ 
script first to produce a \texttt{make.sys}, containing all machine 
dependencies. Then one runs \texttt{make} with appropriate targets.
Possible \configure\ and  \texttt{make} options are documented in the
User Guide.
...
''hand made'' by the makefile using the C preprocessor \texttt{cpp}. 
Normally, \configure\ takes care of this, by selecting the
appropriate case and the producing the appropriate \texttt{make.sys}.
