...
Please apply for an account on qe-forge if you haven't one
...
\texttt{clib/}, \texttt{flib/}, \texttt{Modules/}, \texttt{COUPLE/} 
...
If this is not possible or not convenient, use \#ifdef \_\_MPI
...
and parallel code to a minimum: more \#ifdef \_\_MPI may be needed
only when the flux of parallel and serial execution differ.
\item Unneeded \#ifdef \_\_MPI may be removed if already present;
\#ifdef \_\_PARA is obsolete and must not be used in new developments.
\end{itemize}
...
The following links contain an introduction to \texttt{git}:\\
