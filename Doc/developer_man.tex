...
\section{ Using SVN}
\label{Sec:SVN}
The package is available read-only using anonymous SVN. Developers may
...
it can be very easily installed: see ... 
You will also need a working installation of ssh (secure shell) 
to use SVN.
For the end user, SVN is rather similar to CVS:
if no advanced features are used, the basic commands are the same.
A tutorial can be found here ... 

Follow the instructions in the qe-forge page to check out (i.e.
download) the SVN repository in either read-write or anonymous 
mode.
...
ssn ... checkout
\end{verbatim}
The code appears in directory \texttt{trunk/espresso/}. 
To update the code to the current version: 
\begin{verbatim}
svn update
\end{verbatim}
in the directory containing the distribution.
It is possible to download the version at a
