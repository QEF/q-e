...
\item Suggesting changes: contact developers submitting an ''Issue'' on
  GitLab:\\
		\texttt{git.quantum-espresso.org}, or posting to the 
...
\qe\ is not organized as a monolithic code, but as a
...
as above, with the replacement \texttt{gitlab}$\rightarrow$\texttt{github}.
...
it in the appropriate \texttt{INPUT\_*.def} file; if
...
Stable releases are labelled as $N.M.p$, where $N$=major,
...
Since Release 6.2.1, releases are distributed on GitLab. This is also
the last release distributed on qe-forge.org.
...
\paragraph{Updating web site} After the release is uploaded,
the online documentation must be copied to directory
...
The most important fiels and directories in the root (\texttt{q-e/})
directory are:
\begin{verbatim}
   install/                 configure
   include/                 make.inc
...
\texttt{Doc/}
\item {\em  FFT, Linear Algebra, Utility Libraries}:\\
...
files \texttt{install/m4/*.m4}, then run \autoconf\ to regenerate \configure.
...
\subsection{Merge requests from private repository}
You need to be registered on GitLab but you do not need to have write access
to the repository. You need the following steps:
\begin{itemize}
\item  {\em fork} the QEF/q-e project, using the ``fork'' button
  on \verb|https://gitlab.com/QEF/q-e|
...
\end{itemize}
Once you have done changes in your local copy of the repository,
you have to {\em commit} (save) them:
\begin{quote}
...
\begin{quote}
  \verb|git merge  git@gitlab.com:QEF/q-e.git develop| 
\end{quote}
To align the repository without committing, you may use
\begin{verbatim}
   git pull git@gitlab.com:QEF/q-e.git develop
\end{verbatim}
but if a file is modified both locally and in the repository, a conflict
...
Note: it is good practice to use \verb|git fetch| before a pull or a merge,
as in eg-:\\
\begin{quote}
...
\begin{verbatim}
   git push
\end{verbatim}
Your repository (or more likely, the ``develop'' branch of your repository)
...
\subsection{Merge requests from a branch}
This case requires write access to the QEF/q-e project. Set up your branch
as follows:
\begin{itemize}
\item \verb|git clone git@gitlab.com:QEF/q-e.git| if not already done
\item Make a new branch of the ``develop'' branch. On your copy:\\
  \verb|git checkout -b my-branch QEF/q-e develop|
\item Switch to the new branch:
  \verb|git checkout my-branch|
\end{itemize}
Then work on it as in the previous subsection,
request a merge of ``my-branch'' into ``develop''.

\subsection{Directly into the develop branch}
...
\item Ensure you switch to the ``develop'' branch:
  \verb|git checkout --track origin/develop|
\item Work on it as in the previous subsections
\item When you are ready, \verb|git push|, or, in case of doubt,
  \verb|git push origin develop|.
\end{itemize}
