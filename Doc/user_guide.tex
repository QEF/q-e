...
\configure\ supports out-of-source builds, i.e., it allows to compile the 
binaries in
...
subdirectory.
\texttt{NB:} the usage of a \texttt{build} directory, even though advised for, 
...
in any of the \texttt{Makefile}'s, run \texttt{configure} again in
order to ensure that all \texttt{Makefile}'s and \texttt{make.depend}
are aligned to the current state of the distribution.

It is convenient to use "parallel make" to speed up compilation:
\texttt{make -j [N]} compiles in parallel on N processors if N is
specified, on all available processors otherwise.

Note that if you interrupt \make\ while unpacking and compiling an external 
library, you may run into trouble the next time you type \make. 
If so, run \texttt{make veryclean}, or even \texttt{make distclean}, 
from the top directory, before running \make\ again.
