...
\end{tabular}\\
NOTA BENE: \configure\ no longer updates dependencies. If you modify the
...
NOTA BENE 3: if you interrupt \make\ and you start it again later, it
may fail (this will happen for sure if iinterrupted while unpacking and 
compiling the FoX library). If so, run \texttt{make clean} before
running \make\ again.
...
\paragraph{LIBXC}
\qe\ can use \texttt{libxc} libraries. You need to install \texttt{libxc}
first, then: \verb|configure --with-libxc --with-libxc-prefix=DIR --with-libxc-include=DIR2|,
where DIR is the directory where libxc is installed, DIR2 the directory 
where libxc Fortran headers are. You may look for "libxc" in \texttt{make.inc} 
in case of trouble. Note that currently only a (small) subset of functionals 
implemented in \texttt{libxc} can be used and that the \texttt{libxc}-enabled 
version cannot use functionals from \qe. This will change in the future.
