...
you will need a recent (v.3.14 or later) working CMake software;
otherwise you will need the \configure\ command from autoconf and,
for FoX compilation, \texttt{m4}.
...
development kit (SDK). Ensure that you have properly defined all needed environmental
variables, e.g. by running the "module" file provided in the HPC SDK with command
\texttt{module}. Specify \texttt{--with-cuda} and \texttt{--with-cuda-cc} as explained 
below. This enables OpenMP and disables ScaLAPACK by default. Other options below
