...
\begin{itemize}
  \item The HSE functional was contributed by Hannu-Pekka Komsa 
(CSEA/Lausanne).
...
either to buy a commercial product (e.g Portland, Pathscale, Intel, 
Sun compilers) or to get the open-source compilers gfortran or g95. 
...
subdirectory. Note that both C and Fortran compilers must be in your PATH.
...
clusters, IBM SP and BG machines, SGI Altix, some HP-Compaq Alpha
...
Compiler versions earlier than v.9 are not recommanded, due to the presence
of subtle and insidious bugs.
...
(info from Lucas Fernandez Seivane).

{\bf ifc v.7 and earlier} may still work but are no longer supported.
...
64-bit CPUs like AMD Opteron and Intel Xeon/Itanium are supported and
work both in 32-bit emulation and in 64-bit mode, using either the 
Portland and the Intel compiler. 64-bit executables can address a much
