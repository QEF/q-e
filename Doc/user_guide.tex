...
manual does not cover many important aspects, may contain outdated information.}
...
\begin{itemize}
  ...
  contributions and optimization;
  \item Pietro Bonf\`a (CINECA) for multiple contributions to optimization
  and maiintenance;
  \item Hsin-Yu Ho and Marcos Calegari Andrade (U.Princeton) for SCAN
  and other meta-GGA functionals with \texttt{libxc};
...
section of the web site) in order to post to the  users'
...
Presently, \qe\ is distributed in source form.
% some precompiled executables (binary files) are provided for \texttt{PWgui}.
...
is \version) can be downloaded from the links listed in the
...
&  uncomment \verb|#define __HAVE_CONFIG_INFO| to enable its usage)\\
\end{tabular}\\
...
\end{tabular}\\
(note that ``F90'' is an historical name: we actually use F2003)
...
\texttt{--with-internal-lapack}&  compile with internal LAPACK (default: no)\\
	\texttt{--with-scalapack}&        (yes$|$no$|$intel) Use scalapack if available. \\&Set to \texttt{intel} to use Intel MPI and blacs (default: use OpenMPI)\\
        \texttt{--with-elpa-include}&   Specify full path of ELPA include and modules headers 
	                                (default: no)\\
...
\end{tabular}\\
\\
and the following options targeting the CUDA Fortran accelerated version:\\
\begin{tabular}{ll}
\texttt{--with-cuda=value}&         enables the compiltion of the CUDA Fortran\\
...
Please note that in order to compile the CUDA Fortran code you also need a
recent version of the PGI Fortran compilers (at least 17.4), OpenMP must be enabled,
...
the porrect paths in the \texttt{make.inc} file.
...
allocated: you should never need to recompile your code.
...
\item \texttt{make pwcond} compiles the balistic conductance package 
...
meant to be realistic, implemented only for a subset of codes
\item examples.
Cover many more programs and features of the \qe\ distribution,
but require manual inspection of the results.
\end{itemize}
...
The Intel compiler is nolonger free for personal usage, but it is still
...
Another option is Cygwin, a UNIX environment which runs under Windows:
