...
\texttt{XClib/}   &  Exchange-correlation functionals (not van der Waals)\\ 
...
You will also need CMake if you prefer installation with CMake (supported 
since v.6.7), otherwise you will need the \configure\ command from autoconf
(see next subsection).
...
CUDA Fortran version and to compile with the NVidia compiler and
...
you need either to buy a commercial product (e.g the Intel compiler) 
or to use the open-source gfortran  compiler from the gcc distribution.
Some commercial compilers may be available free of charge 
...
you also need a recent version of the NVidia Fortran compiler. 
