...
\item \texttt{make gamma} produces \texttt{Gamma/phcg.x}: 
  ...
  tem  \texttt{make pp} produces several codes for data postprocessing, in 
...
\item  \texttt{path\_int.sh}/ \texttt{path\_int.x}: utility to generate, starting from a
...
but with a different calling sequence. The appearence of run-time errors like {\em
...
g95 v.0.91 and later (\texttt{http://www.g95.org}) works flawlessy. 
...
ifort versions $<9.1$ are not recommanded, due to the presence of subtle 
...
\begin{verbatim}
   ...
   port install g95
\end{verbatim}
The apple supplied mpi environment has to be overriden since there is
...
openmpi overwrites apple supplied mpi permenantly!\\
Next step is the Quantum-Espresso itself. Sadly, the Apple suplied accelaration
libraries tend to misbehave under different tests, and it is much safer to
use internally supplied replacements. The minimum recommended configuration
...
The recommended compilation line for the Quantum Espresso is: 
\begin{verbatim}
 ./configure CC=icc CXX=icpc F77=ifort F90=ifort FC=ifort CPP=cpp-mp-4.3
\end{verbatim}
...
$n^2$ must be smaller than the number of precessors of a single pool.
...
into trouble (or into angry system managers) with eccessive I/O with \pw.x. 
...
Note about XC: the type of XC used in the calculation is read from PP files. 
All PPs must have been generated using the same XC. You can override
this choice by setting input variable \texttt{input\_dft} (see list of
allowed values in \texttt{Modules/funct.f90}).
...
self-consistentcy in a \pw.x\ run.
...
the equilibrium positions, then you need to dispacee ions from the equilibrium 
...
\paragraph{\pw.x stops with \texttt{internal error: cannot braket Ef}}
Possible reasons:
\begin{itemize}
...
\paragraph{\pw.x yields \texttt{internal error: cannot braket Ef} message 
but does not stop} 
...
If resutlts are really weird (as opposed to misinterpreted):
\begin{itemize}
...
carefullt check your structure first, e.g. using XCrySDen.
...
the generation of the PP is the same for all PPs. Note that 
