...
\begin{itemize}
  ...
  and other meta-GG functionals with \texttt{libxc};
...
\end{tabular}\\
\\
and the following options targeting the CUDA Fortan accelerated version:\\
\begin{tabular}{ll}
\texttt{--with-cuda=value}&         enables CUDA Fortran accelerated subroutines. \\
                          &         \texttt{value} should point the path where \\
                          &         the CUDA Runtime is installed (default: no)\\
\texttt{--with-cuda-cc=value}&      sets the compute capabilities for the compilation\\
                             &      of the accelerated subroutines. \\
                             &      \texttt{value} should be consistent with the \\
                             &      hardware and the NVidia driver installed on the workstation \\
                             &      or the compute nodes of the HPC facility  (default: 35)\\
\texttt{--with-cuda-runtime=value}& sets the version of the CUDA runtime used \\
                                  & for the compilation of the accelerated code.\\
                                  & \texttt{value} should be consistent with the\\
                                  & CUDA Toolkit installed on the workstation \\
                                  & or available on compute nodes of the HPC facility.\\
                                  & PGI compilers currently accept 7.5, 8.0 or 9.0.  (default: 8.0)\\
\end{tabular}\\

Please note that in order to compile the CUDA Fortran code you need a
recent version of the PGI Fortran compiler, you need to enable OpenMP,
