...
\end{verbatim}
(Info by Paolo Giannozzi, jan.2008, updated for v.4.2 and following)
...
value using the command line switch {\texttt-ndiag} for \pw.x,
\texttt{-northo} for \cp.x.
...
\texttt{ndiag} is chosen
by the code as the largest $n^2$ (integer $n$) that fits into the size
of each pool.
...
\texttt{-D\_\_SCALAPACK} to DFLAGS in \texttt{make.sys} and you have to  
modify the LAPACK\_LIBS variable as in the following example:
\begin{verbatim}
LAPACK_LIBS = -lscalapack -lblacs -lblacsF77init -lblacs -llapack
\end{verbatim}
The repeated \texttt{-lblacs} is not an error, it is needed! after Scalapack,
...
using the same exchange-correlation. You can override this choice
by setting input variable \texttt{input\_dft} (see list of
...
are in files \texttt{Doc/INPUT\_PW.*}. Please read them carefully. There is a large number of other
...
\PWscf.
See files \texttt{Doc/INPUT\_PW.*} for a detailed explanation of the meaning and format
