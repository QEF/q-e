...
!
      ...
      if (.not.numeric) then
         ...
         if ( abs(cc(1)+cc(2)-1.d0).gt.1.0d-6) call errore  &
         ...
         if (nlcc) then
            read( iunps, *, err=300, iostat=ios ) a_core,  &
                b_core, alfa_core
            if (alfa_core.le.0.d0)  &
                 call errore('readin','nlcc but alfa=0',1)
         endif
         if (cc(2).ne.0.d0.and.alpc(2).ne.0.d0.and.bhstype) then
...
!
               vnl(ir,l) = 2.d0*vnloc-2.d0*zval/r(ir)*(cc(1)*  &
