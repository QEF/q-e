...
200 return  
  ...
  subroutine read_pseudo_upf_rel (iunps, upf, ierr)  
    !---------------------------------------------------------------------
    !
    !   read pseudopotential "upf" in the Unified Pseudopotential Format
    !   from unit "iunps" - return error code in "ierr" (success: ierr=0)
  ...
  end subroutine read_pseudo_upf_rel
  !---------------------------------------------------------------------
...
100 call errore ('read_pseudo_local','Reading pseudo file', abs(ios) )
  ...
  subroutine read_pseudo_nl (upf, iunps)  
    ...
    do nb = 1, upf%nbeta 
       ...
       read (iunps, *, err = 100, iostat = ios) idum, &
            &    upf%lll(nb), dummy
...
100 call errore ('read_pseudo_addinfo', 'Add_info not present', -abs(ios))   
    return
  end subroutine read_pseudo_addinfo

  !=----------------------------------------------------------------------------=!
END MODULE read_pseudo_module_ld1
!=----------------------------------------------------------------------------=!
