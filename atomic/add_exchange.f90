...
!            if (doc /= 0.d0) write (*,*) "DOC ",nu, doc
         ...
         do l3=l0,l1+l2
            ...
            if (abs(sss).gt.1.0d-10) then
               call hartree(l3,l1+l2+2,mesh,r,r2,sqr,dx,wrk,wrk2)
...
!- spurious hartree part 
          if (mu.eq.nu) then
            call hartree(0,2,mesh,r,r2,sqr,dx,wrk,wrk2)
            fac = doc*e2
            do i=1,mesh
               wrk1(i) = wrk1(i) + fac*wrk2(i)*wrk(i)
            end do
...
!
         nst = 2 * min(l1,l2) + 2
         sxc = sxc + int_0_inf_dr(wrk1,r,r2,dx,mesh,nst)
      end do 
!
      do i=1,mesh
         wrk1(i) = vx(i,is)*psi(i,1,nu)*psi(i,1,nu)
      end do
      sxc1 = int_0_inf_dr(wrk1,r,r2,dx,mesh,2*ll(nu)+2)
