...
subroutine scf
  ...
  real(kind=dp)::  vnew(ndm,2), rhoc1(ndm), vhn1(ndm), egc(ndm), rab(ndm), &
       ze2, enl1(nwfx)
  ...
  do iter=1,maxter
     ...
     do n=1,nwf
        if (oc(n).ge.0.d0) then
           ...
           !      write(6,*) el(n),enl(n)
           if (nstop.ne.0) write(6,'(4i6)') iter,nn(n),ll(n),nstop
           nerr=nerr+nstop
        else
           enl(n)=0.d0
           psi(:,n)=0.d0
     ...
     if (conv) then
        if (nerr.ne.0) call errore('scf_nr','errors in cscheq',-1)
        goto 45
     endif
  enddo
  call errore('scf_nr','warning: convergence not achieved',-1)
45 if (isic.ne.0) then
     deallocate(vsicnew)
     deallocate(vsic)
  endif
