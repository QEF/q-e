...
!
      do n=ik+1,2,-1
         yi(n-1)=((12.d0-10.d0*f(n))*yi(n)-f(n+1)*yi(n+1))/f(n-1)
      enddo
!
         ...
         if (lls(ib).eq.lam.and.jjs(ib).eq.jam) then
            ...
            do n=2,nstart-1,2
                 c(n+1)=c(n-1)+(el(n-1)*r(n-1)+ &
                       4.d0*el(n)*r(n) + el(n+1)*r(n+1))*dx/3.d0
...
!    symmetrize b
!
!      write(6,*) iib,2.d0*bm(1,1),ddd(1,1),qq(1,1)
      do ib=1,iib
         do jb=1,ib
...
!      write(6,*) 'amat alla fine',iib,amat(1,1)
      ...
      if (iib.eq.1) then
         det= amat(1,1)-1.d0
      elseif (iib.eq.2) then
         det=(amat(1,1)-1.d0)*(amat(2,2)-1.d0)-amat(2,1)*amat(1,2)
      elseif (iib.eq.0) then
         det=wron1
      else
