...
#include"../include/fpmd.h"
      ...
      DIMENSION a(ld,n),u(ld,n),v(ld,n),work(4*n)
      zero=0.0d0
      one=1.0d0
...
! ... scale matrix before inversion
      IF (iopt.GE.10) THEN
        ...
        DO i=1,n
          work(n3+i)=one
          IF (dabs(a(i,i)).GE.1.d-13) &
             work(n3+i)=one/dsqrt(dabs(a(i,i)))
        END DO
        DO i=1,n
          DO j=1,n
...
! ... get singular values
      ...
      mrank=0
      DO i=1,n
        IF (dabs(work(i)).GT.toleig) mrank=mrank+1
      END DO
...
!
      s(l)=FPMD_DNRM2(n-l+1,x(l,l),1)
            IF (S(L) .EQ. 0.0D0) GO TO 10
               IF (X(L,L) .NE. 0.0D0) S(L) = SIGN(S(L),X(L,L))
      call FPMD_DSCAL(n-l+1,1.0d0/s(l),x(l,l),1)
               X(L,L) = 1.0D0 + X(L,L)
   10       CONTINUE
            S(L) = -S(L)
   20    CONTINUE
...
!
      t=-FPMD_DDOT(n-l+1,x(l,l),1,x(l,j),1)/x(l,l)
      call FPMD_DAXPY(n-l+1,t,x(l,l),1,x(l,j),1)
   30       CONTINUE
!
...
!
      e(l)=FPMD_DNRM2(p-l,e(lp1),1)
            IF (E(L) .EQ. 0.0D0) GO TO 80
               IF (E(LP1) .NE. 0.0D0) E(L) = SIGN(E(L),E(LP1))
      call FPMD_DSCAL(p-l,1.0d0/e(l),e(lp1),1)
               E(LP1) = 1.0D0 + E(LP1)
   80       CONTINUE
            E(L) = -E(L)
...
!
   ...
   90          CONTINUE
               DO 100 J = LP1, P
      call FPMD_DAXPY(n-l,e(j),x(lp1,j),1,work(lp1),1)
  100          CONTINUE
               DO 110 J = LP1, P
      call FPMD_DAXPY(n-l,-e(j)/e(lp1),work(lp1),1,x(lp1,j),1)
  110          CONTINUE
  120       CONTINUE
  ...
  200    CONTINUE
         ...
         DO 280 LL = 1, NCT
            ...
            IF (S(L) .EQ. 0.0D0) GO TO 250
               ...
               DO 210 J = LP1, NCU
               t=-FPMD_DDOT(n-l+1,u(l,l),1,u(l,j),1)/u(l,l)
               call FPMD_DAXPY(n-l+1,t,u(l,l),1,u(l,j),1)
  210          CONTINUE
  220          CONTINUE
               call FPMD_DSCAL(n-l+1,-1.0d0,u(l,l),1)
               U(L,L) = 1.0D0 + U(L,L)
               LM1 = L - 1
      ...
      IF (.NOT.WANTV) GO TO 350
         DO 340 LL = 1, P
            ...
            IF (E(L) .EQ. 0.0D0) GO TO 320
               DO 310 J = LP1, P
               t=-FPMD_DDOT(p-l,v(lp1,l),1,v(lp1,j),1)/v(lp1,l)
               call FPMD_DAXPY(p-l,t,v(lp1,l),1,v(lp1,j),1)
  310          CONTINUE
  320       CONTINUE
            DO 330 I = 1, P
               V(I,L) = 0.0D0
...
!
            IF (S(L) .GE. 0.0D0) GO TO 580
               S(L) = -S(L)
               if(wantv)call FPMD_DSCAL(p,-1.0d0,v(1,l),1)
  580       CONTINUE
!
...
!           ...EXIT
               ...
               S(L) = S(L+1)
               S(L+1) = T
      if(wantv.and.l.lt.p)call FPMD_DSWAP(p,v(1,l),1,v(1,l+1),1)
      if(wantu.and.l.lt.n)call FPMD_DSWAP(n,u(1,l),1,u(1,l+1),1)
               L = L + 1
            GO TO 590
  600       CONTINUE
            ITER = 0
...
!
      USE kinds
      REAL(dbl) DA,DB,C,S,ROE,SCALE,R,Z
!
      ROE = DB
