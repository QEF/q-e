...
C                EIGENVALUES IN INTERVAL ..........
 ...
 1001 LB = T1
      ...
      RETURN
      END
      SUBROUTINE CHG(NM,N,AR,AI,BR,BI,W,MATZ,ZR,ZI,FV1,FV2,FV3,FM1,IERR)
C
      INTEGER N,NM,IERR,MATZ
      DOUBLE PRECISION AR(NM,N),AI(NM,N),BR(NM,N),BI(NM,N),W(N),ZR(NM,N)
     $ ,ZI(NM,N),FV1(N),FV2(N),FV3(N),FM1(2,N)
C
C     THIS SUBROUTINE CALLS THE RECOMMENDED SEQUENCE OF SUBROUTINES
C     FROM THE EIGENSYSTEM SUBROUTINE PACKAGE(EISPAK) AND EXTENSIONS
C     TO IT, FOR THE COMPLEX HERMITEAN GENERALIZED EIGENPROBLEM
C     AX=(LAMBDA)BX.
C
C     ON INPUT
C
...
C         DIMENSION STATEMENT
C
C       N IS THE ORDER OF MATRICES A=(AR,AI), B=(BR,BI)
C
C       AR,AI CONTAIN THE REAL AND IMAGINARY PARTS OF A COMPLEX
C         HERMITEAN MATRIX
C
C       BR,BI CONTAIN THE REAL AND IMAGINARY PARTS OF A POSITIVE
C         DEFINITE COMPLEX HERMITEAN MATRIX
C
C       MATZ IS AN INTEGER VARIABLE SET EQUAL TO ZERO IF ONLY
C         EIGENVALUES ARE DESIRED. OTHERWISE IT IS SET TO ANY NON-ZERO
C         INTEGER FOR BOTH EIGENVALUES AND EIGENVECTORS
C
C     ON OUTPUT
C
C       W CONTAINS THE EIGENVALUES IN ASCENDING ORDER
C
C       ZR,ZI CONTAINS THE REAL AND IMAGINARY PARTS OF THE EIGENVECTORS
C         IF MATZ IS NOT ZERO
C
C        IERR  IS AN INTEGER OUTPUT VARIABLE SET EQUAL TO AN ERROR
C           COMPLETION CODE DESCRIBED IN THE DOCUMENTATION FOR TQLRAT,
C           HREDUC AND TQL2.  THE NORMAL COMPLETION CODE IS ZERO.
C
C     FV1,FV2,FV3,FM1 ARE TEMPORARY STORAGE ARRAYS.
C
C          THIS ROUTINE HAS BEEN DEVELOPED BY ALVISE NOBILE, SISSA,
C          STR. COSTIERA 11,TRIESTE,ITALY. QUESTIONS AND COMMENTS
C          SHOULD BE DIRECTED TO THE AUTHOR.
C
C          THIS VERSION DATED MARCH 1985.
C
C     ------------------------------------------------------------------
C
      IF( N .LE. NM ) GOTO 10
      ...
      GOTO 50
C
10    CALL HREDUC(NM,N,AR,AI,BR,BI,FV3,IERR)
...
C
      ...
      IF( IERR .NE. 0 ) GOTO 50
      CALL HTRIBK(NM,N,AR,AI,FM1,N,ZR,ZI)
      CALL HREBAK(NM,N,BR,BI,FV3,N,ZR,ZI)
50    RETURN
      END
C
C      ROUTINES SVILUPPATE ALLA SISSA
C
      SUBROUTINE HREBAK(NM,N,BR,BI,DL,M,ZR,ZI)
C
      INTEGER I,J,K,M,N,NM,I1,J1
      DOUBLE PRECISION BR(NM,N),BI(NM,N),DL(N),ZR(NM,M),ZI(NM,M)
      DOUBLE PRECISION XR,XI
C
C     THIS SUBROUTINE IS A COMPLEX ANALOG OF ROUTINE REBAK OF EISPAK.
C     SEE DOCUMENTATION OF REBAK FOR REFERENCES TO THE ALGORITHM.
C
C     THIS ROUTINE FORMS THE EIGENVECTORS OF A GENERALIZED HERMITEAN
C     EIGENSYSTEM BY BACKTRANSFORMING THOSE OF THE DERIVED HERMITEAN
C     MATRIX DETERMINED BY HREDUC
C
C     ON INPUT
C
C       NM MUST BE SET TO THE ROW DIMENSION OF THE TWO-DIMENSIONAL
...
C       N IS THE ORDER OF THE MATRIX SYSTEM
C
C       BR,BI CONTAIN INFORMATION ON THE SIMILARITY TRANSFORMATION
C         ( CHOLESKY FACTORIZATION) USED IN THE REDUCTION BY HREDUC
C         IN THEIR STRICT LOWER TRIANGLES.
C
C       M IS THE NUMBER OF EIGENVECTORS TO BACKTRANSFORM
C
C       ZR,ZI CONTAIN THE REAL AND IMAGINARY PARTS OF THE EIGENVECTORS
C         TO BE BACKTRANSFORMED IN THEIR FIRST M COLUMNS.
C
C     ON OUTPUT
C
...
200   RETURN
      END
      SUBROUTINE HREDUC(NM,N,AR,AI,BR,BI,DL,IERR)
C
      INTEGER  I,J,K,N,I1,J1,NM,NN,IERR
      DOUBLE PRECISION AR(NM,N),BR(NM,N),AI(NM,N),BI(NM,N),DL(N),XR,XI,Y
C
C     THIS SUBROUTINE IS A COMPLEX ANALOG OF ROUTINE REDUC OF EISPAK.
C     SEE DOCUMENTATION OF REDUC FOR REFERENCES TO THE ALGORITHM.
C
C        THIS SUBROUTINE REDUCES THE GENERALIZED HERMITIAN EIGENPROBLEM
C        AX=(LAMBDA)BX, WHERE B IS POSITIVE DEFINITE, TO THE STANDARD
C        SYMMETRIC EIGENPROBLEM USING THE CHOLESKY FACTORIZATION OF B.
C        ON INPUT
...
C             STATEMENT
C
C          N  IS THE ORDER OF THE MATRICES AR,AI,BR,BI. IF THE CHOLESKY
C             FACTOR (LR,LI) OF (BR,BI) IS ALREADY AVAILABLE, N SHOULD
C             BE PREFIXED WITH A MINUS SIGN
C
C          AR,AI,BR,BI CONTAIN RESPECTIVELY THE REAL (AR,BR) AND
C             IMAGINARY(AI,BI) PARTS OF THE INPUT HERMITEAN MATRICES.
C             ONLY FULL UPPER TRIANGLE OF THE MATRICES NEED BE SUPPLIED.
C             IF N IS NEGATIVE, THE STRICT LOWER TRIANGLES OF BR,BI
C             CONTAIN INSTEAD THE REAL AND IMAGINARY PARTS OF THE STRICT
C             LOWER TRIANGLE OF ITS CHOLESKY FACTOR L.
C
C          DL CONTAINS, IF N IS NEGATIVE, THE REAL PART OF THE DIAGONAL
C             ELEMENTS OF L.
C
C        ON OUTPUT
C
C          AR,AI CONTAIN IN THEIR FULL LOWER TRIANGLE THE FULL LOWER
C             TRIANGLE OF THE HERMITEAN MATRIX DERIVED FROM THE
C             REDUCTION TO THE STANDARD FORM. THE STRICT UPPER TRIANGLES
C             OF AR,AI ARE UNALTERED.
C
C          BR,BI CONTAIN IN THEIR STRICT LOWER TRIANGLES THE REAL
C             AND IMAGINARY PARTS OF THE STRICT LOWER TRIANGLE OF THE
C             CHOLESKY FACTOR L . THE FULL UPPER TRIANGLES OF BR,BI ARE
C             UNALTERED.
C
C          DL CONTAINS THE REAL PARTS OF THE DIAGONAL ELEMENTS OF L
C
C          IERR IS SET TO
C            ZERO       FOR NORMAL RETURN
C            7*N+1      IF B IS NOT POSITIVE DEFINITE
...
C
      ...
      DO 80 I=1,N
         ...
         DO 40 J=1,I1
            ...
            IF( J1 .EQ. 0 ) GOTO 30
            DO 20 K = 1,J1
               XR = XR - BR(J,K) * BR(I,K) - BI(J,K) * BI(I,K)
               XI = XI - BR(J,K) * BI(I,K) + BI(J,K) * BR(I,K)
20          CONTINUE
30          CONTINUE
            BR(I,J) = XR / DL(J)
            BI(I,J) = XI / DL(J)
...
50       CONTINUE
         ...
         IF( I1 .EQ. 0 ) GOTO 70
         DO 60 K = 1,I1
            XR = XR - BR(I,K)**2 - BI(I,K)**2
60       CONTINUE
70       CONTINUE
         IF( XR .LE. 0.0 ) GOTO 1000
         DL(I) = SQRT( XR )
80    CONTINUE
C
C     FORM THE HERMITIAN CONJUGATE OF THE UPPER TRIANGLE OF
C     INV(L)*A IN THE LOWER TRIANLE OF A
C
100   DO 200 I=1,NN
         I1 = I - 1
         Y = DL(I)
