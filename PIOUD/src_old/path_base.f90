...
MODULE path_base
  ...
  !!    END IF
    ...
    SUBROUTINE search_mep()
      ...
      !!! USE path_reparametrisation   !!! <----my mod.
      !
      IMPLICIT NONE
      !
      LOGICAL :: stat
      ...
      optimisation: DO
       ...
       !!!  CALL write_restart()   !!! <----my mod.
         ...
         !
       !!!  IF ( .NOT. fixed_tan ) tangent(:,:) = new_tangent()   !!! <----my mod.
         !
       !!!  IF ( CI_scheme == "auto" ) THEN    !!! <----my mod.
            !
       !!!     climbing = .FALSE.    !!! <----my mod.
            !
        !!!    climbing(Emax_index) = .TRUE.    !!! <----my mod.
            !
       !!!  END IF   !!! <----my mod.
         !
        !!! IF ( lneb ) CALL neb_gradient()   !!! <----my mod.
        !!! IF ( lsmd ) CALL smd_gradient()   !!! <----my mod.
         !
         ! ... the forward activation energy is computed here
         !
       !!!  activation_energy = ( pes(Emax_index) - pes(1) )*autoev   !!! <----my mod.
         !
         ! ... the error is computed here (frozen images are also set here)
         !
       !!!  CALL compute_error( err_max )    !!! <----my mod.
       !!!  IF ( lfcpopt ) CALL fcp_compute_error( fcp_err_max )    !!! <----my mod.
         !
      ...
      !!!   CALL write_restart()    !!! <----my mod.
         ...
         ! ... exit conditions
         !
     !!!    IF ( check_exit( err_max, fcp_err_max ) ) EXIT optimisation   !!! <----my mod.
         !
         ! ... if convergence is not yet achieved, the path is optimised
         !
     !!!    CALL optimisation_step()   !!! <----my mod.
         !
         IF (meta_ionode)    call pimd_pw_convert_pos('pw_to_md')    !!! <----my mod. 
