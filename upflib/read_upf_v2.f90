...
CONTAINS
   ...
   !---------------------------------------------+
      ...
      !! within in the PP_INFO/PP_INPUT section some xml reserved characters which 
      ...
      !
         IF(.not. present(ierr)) &
            CALL upf_error('read_upf_v2','Cannot open UPF file.',1)
         ierr = 1
         RETURN
      ENDIF
      IF ( .not. matches('UPF',root) ) THEN
         IF (PRESENT (ierr) ) THEN 
            CALL upf_error( 'read_upf_v2', 'tagname is '//TRIM(root)//' instead of UPF', -1 )
            ierr = 2 
            RETURN
         ELSE 
            CALL upf_error('read_upf_v2', 'tagname is '//TRIM(root)//' instead of UPF',2)
         END IF
      END IF
      CALL extractDataAttribute(u, 'version', upf%nv)
      IF (version_compare(upf%nv, max_version) == 'newer') &
         CALL upf_error('read_upf_v2', 'Unknown UPF format version: '//TRIM(upf%nv),1)
      !
      !
      ...
      IF ( version_compare(upf%nv, '2.0.1') == 'older' .and. upf%tvanp .and.  &
         .not. upf%tpawp ) CALL upf_error('read_upf_v2',&
...
!        
   CALL extractDataAttribute(hdrNode, 'functional',  dft_buffer)
      ...
      IF (len_buffer > len(upf%dft)) &
         CALL upf_error('read_upf_v2','String defining DFT is too long',len_buffer)
...
!
   ...
   SUBROUTINE read_upf_pswfc(u, upf)
      ...
      DO  
         ...
         IF ( nw .GT. upf%nwfc ) THEN 
            CALL upf_error('pseudo '//trim(upf%psd), "too many chi found in pswfc",-1 ) 
            EXIT
         END IF
         IF ( hasAttribute (locNode, 'label')) THEN 
   ...
   SUBROUTINE read_upf_paw(u, upf)
      ...
      IF(upf%paw_data_format /= 2) &
         CALL upf_error('read_upf_v2::paw',&
...
!
   SUBROUTINE read_upf_gipaw(u, upf)
      ...
      IF(upf%gipaw_data_format /= 2) &
         CALL upf_error('read_upf_v2::gipaw','Unknown format version',-1)
      !
