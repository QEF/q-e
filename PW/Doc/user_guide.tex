...
This guide covers the installation and usage of \PWscf\ (opEn-Source 
...
\texttt{NEB} reaction pathways and energy barriers using the Nudged Elastic Band (NEB) 
and Fourier String Method Dynamics (SMD) methods are no longer performed
by the \pwx\ executable of \PWscf. 

\subsection{People and terms of use}
...
\PWscf\ is included inside the \qe\ distribution.


\subsection{Prerequisites}

To install \PWscf\ from source, you need first of all a minimal Unix
...
After the correct installation of \qe \, the installation and compilation of \PWscf \
can be done just typing \texttt{make pw} from the main espresso directory.

\subsection{Compilation}

Typing \texttt{make pw} from the main \qe\ directory or
typing \texttt{make} from the \texttt{PW} directory  will produce
the following codes and program utilities in \texttt{PW/src} and in PW/tools:

\begin{itemize}
\item \texttt{PW/pw.x} calculates electronic structure, structural optimization, molecular dynamics. \\
...
\texttt{PW/tests/}. File \texttt{PW/tests/README} contains a list of what is tested.
...
in \PWscf:
\begin{enumerate}
\item {\em Message-Passing (MPI)}.  
\item {\em OpenMP}. 
\end{enumerate}

In \PWscf\ several parallelization levels, specified at run-time
...
{\bf pools}: When k-point sampling is used, "world" can be 
...
command line switches: \texttt{-npools},
\texttt{-ntg}, \texttt{-northo} are used.
As an example consider the following command line:
\begin{verbatim}
mpirun -np 4096 ./pw.x -nimage 8 -npool 2 -ntg 8 -northo 144 -input my.input
\end{verbatim}
...
Default values are: \texttt{-npool 1 -ntg 1} ; 
