...
#include "machine.h"
  ...
  real(kind=DP), pointer :: gk (:,:), qg (:), vq (:), ylm (:,:), vkb1(:,:)  
  ...
  call mallocate(vkb1, npw_,nhm)
  call mallocate(  sk, npw_)
  call mallocate(  qg, npw_)  
  call mallocate(  vq, npw_)  
  call mallocate( ylm, npw_, (lmaxkb + 1) **2)  
  call mallocate(  gk, 3, npw_)
  !
  do ig = 1, npw_  
