...
#include "machine.h"
  ...
  ! idem
  complex(kind=DP), pointer :: vc (:,:), vcc (:,:),&
       rmat (:,:,:), smat (:,:,:), psip (:,:), psis (:,:), &
  ...
  real(kind=DP), pointer :: nrm (:), ew (:), ep (:), np (:), ec (:)
  ...
  call mallocate(vc,  diis_steps , diis_steps)  
  call mallocate(vcc, diis_steps , buflen)  
  call mallocate(psip,  ndmx , buflen)  
  call mallocate(psis,  ndmx , buflen)  
  call mallocate(pres,  ndmx , buflen)  
  call mallocate(dwfc,  ndmx , diis_steps * buflen)  
  call mallocate(dres,  ndmx , diis_steps * buflen)  
  call mallocate(ec, diis_steps)  
  call mallocate(ep, nvec)  
  call mallocate(ew, buflen)  
  call mallocate(et, 2 * nvec)  
  call mallocate(np, nvec)  
  call mallocate(eau, buflen)  
  call mallocate(nrm, buflen)  
  call mallocate(rmat,  diis_steps,  diis_steps,  buflen)  
  call mallocate(smat,  diis_steps,  diis_steps,  buflen)  
  call mallocate(conv, nvec)  
  call mallocate(cstep, buflen)  
  call mallocate(buf2wfc,  buflen * diis_steps)  
  call mallocate(wfc2buf,  buflen , diis_steps)  
