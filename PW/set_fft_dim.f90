...
subroutine set_fft_dim
  ...
  if (nr1.eq.0) then
     ...
     nr1 = int (2 * sqrt (gcutm) * sqrt (at (1, 1) **2 + at (2, 1) &
          **2 + at (3, 1) **2) ) + 1
10   continue
     if (nr1.gt.nmax) call errore ('set_fft_dim', 'nr1 > nmax ?!? ', &
          nr1)
...
11   continue
     if (nr1s.gt.nmax) call errore ('set_fft_dim', 'nr1s > nmax ?!? ', nr1s)
...
16 continue
  ...
  if (nr2.eq.0) then
     ...
     nr2 = int (2 * sqrt (gcutm) * sqrt (at (1, 2) **2 + at (2, 2) &
          **2 + at (3, 2) **2) ) + 1
20   continue
     if (nr2.gt.nmax) call errore ('set_fft_dim', 'nr2 > nmax ?!? ', nr2)
...
21   continue
     if (nr2s.gt.nmax) call errore ('set_fft_dim', 'nr2s > nmax ?!? ', nr2s)
...
26 continue
  ...
  if (nr3.eq.0) then
     ...
     nr3 = int (2 * sqrt (gcutm) * sqrt (at (1, 3) **2 + at (2, 3) &
          **2 + at (3, 3) **2) ) + 1
30   continue
     if (nr3.gt.nmax) call errore ('set_fft_dim', 'nr3 > nmax ?!? ', nr3)
...
31   continue
     if (nr3s.gt.nmax) call errore ('set_fft_dim', 'nr3s > nmax ?!? ', &
          &nr3s)
...
36 continue
  ...
  if (.not.doublegrid.and. ( &
       ...
       call errore ('set_fft_dim', 'grids and dual are inconsistent', 2)
  return
