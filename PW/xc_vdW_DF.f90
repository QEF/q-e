...
MODULE vdW_DF
  ...
  USE kernel_table,      ONLY : q_mesh, Nr_points, Nqs, initialize_kernel_table, r_max
...
!!                                       |_____________|
  ...
  SUBROUTINE xc_vdW_DF(rho_valence, rho_core, etxc, vtxc, v)
    ...
    if (first_iteration) then
       ...
       ! This routine reads the kernel table "vdW_kernel_table".  It looks first in the current
       ! directory.  If the kernel table is not found there it checks the pseudopotential 
       ! directory.  If there is no kernel table there it defaults to the one provided in the
       ! PW directory of the PWSCF source tree.  It also defines several variables that are needed
       ! by various routines below (Nqs=number of q points, Nr_points=number of radial points,
       ! r_max=value of largest radial point, q_mesh=an array holding all the q points chosen
       ! for this particular run, kernel=an array holding the Fourier transformed kernel values at all the 
       ! q_mesh points, d2phi_dk2=the second derivatives of the Fourier transformed kernel required to 
       ! do kernel interpolations).  See the kernel_table.f90 file for more details.
