...
SUBROUTINE stres_us_gpu( ik, gk, sigmanlc )
  ...
  ALLOCATE( ix(nat*nhm,4) )
  !$acc data create(is_multinp,ix) copyin(ityp,nh)
  !
  ALLOCATE( shift(nat) )
  ijkb01 = 0
  DO iu = 1, ntyp
...
10     CONTINUE
     ...
     SUBROUTINE stres_us_k_gpu()
       ...
       !
#if defined(__CUDA) 
       ATTRIBUTES(DEVICE) :: becpnc_d, becpk_d
#endif
       !
       !
#if defined(__CUDA)
         ...
         IF ( noncolin ) THEN
            ...
            DO i = 1, itot
              ...
              IF (.NOT. is_multinp(i)) THEN
                 ...
                 DO is = 1, npol
                   ijs = (is-1)*npol
                   ps_nc(ikb,is) = SUM( becpnc_d(ikb,1:npol,ibnd) * &
              ...
              ELSE
                 ...
                 DO is = 1, npol
                   ijs = (is-1)*npol
                   ps_nc(ikb,is) = SUM( becpnc_d(ishift+1:ishift+nh_np,1:npol,ibnd) * &
         ...
         ELSE
            ...
            DO i = 1, itot
               ...
               ELSE 
                  nh_np = ix(i,3)
                  !
                  ps(ikb) = SUM( becpk_d(ishift+1:ishift+nh_np,ibnd) * &
                                               deff(ih,1:nh_np,na) )
               ENDIF
               !
            ENDDO
