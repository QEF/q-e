...
!
!----------------------------------------------------------------------------
SUBROUTINE add_vuspsi_omp( lda, n, m, hpsi )
  !----------------------------------------------------------------------------
  ...
  IMPLICIT NONE
  !
  ! This is a temporary interface to offload add_vuspsi with openmp
  ...
  !
  !
  CALL start_clock( 'add_vuspsi_omp' )  
  CALL using_becp_auto(0)
  !
  IF ( gamma_only ) THEN
     !
     CALL add_vuspsi_gamma_omp()
     !
  ELSE IF ( noncolin) THEN
     !
     CALL add_vuspsi_nc_omp()
     !
  ELSE
     !
     CALL add_vuspsi_k_omp()
     !
  ENDIF
  !
  CALL stop_clock( 'add_vuspsi_omp' )  
  !
  RETURN
  !
  CONTAINS
     !
     !-----------------------------------------------------------------------
     SUBROUTINE add_vuspsi_gamma_omp()
       !-----------------------------------------------------------------------
       !! See comments inside
       !
       ...
       RETURN
       !
     END SUBROUTINE add_vuspsi_gamma_omp
     !
     !-----------------------------------------------------------------------
     SUBROUTINE add_vuspsi_k_omp()
       !-----------------------------------------------------------------------
       ...
       RETURN
       !
     END SUBROUTINE add_vuspsi_k_omp     
     !  
     !-----------------------------------------------------------------------
     SUBROUTINE add_vuspsi_nc_omp()
       !-----------------------------------------------------------------------
       ...
       RETURN
       !
     END SUBROUTINE add_vuspsi_nc_omp
     !  
     !  
END SUBROUTINE add_vuspsi_omp
