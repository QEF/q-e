   ...
   atoms: DO ia = ia_s, ia_e
      ...
      ifpaw: IF (upf(i%t)%tpawp) THEN
         ...
         whattodo: DO i_what = AE, PS
            ...
            IF (i_what == AE) THEN
               ...
               IF (nspin_mag==4.and.upf(i%t)%has_so) THEN
      ...
      ! NOTE: optional variables works recursively: e.g. if energy is not present here
            ...
            spins: DO is = 1, nspin_mag
               ...
               DO nb = 1, nh(i%t)
                  DO mb = nb, nh(i%t)
                     ...
                     IF (i_what == AE) THEN
                        ...
                        IF (nspin_mag==4.and.upf(i%t)%has_so) &
                     ...
                     DO lm = 1, l2
                        ...
                        IF (is>1.and.nspin_mag==4.AND.upf(i%t)%has_so.AND.&
                                             i_what== AE ) THEN
                           DO j=1, imesh
               ...
               ENDDO ! nb
            ENDDO spins
            IF (nspin_mag==4.AND.upf(i%t)%has_so.AND.i_what== AE ) THEN
...
SUBROUTINE PAW_xc_potential(i, rho_lm, rho_core, v_lm, energy)
    ...
    REAL(DP)              :: g_rad(i%m,rad(i%t)%nx,nspin) ! radial potential
...
!$omp shared(i,rad,v_lm,rho_lm,rho_core,v_rad,ix_s,ix_e,energy,e_of_tid,nspin,g,lsd)
    ...
    IF (upf(i%t)%has_so.and.nspin_mag==4.and.i%ae==1) &
         ALLOCATE( msmall_rad(i%m,nspin_mag) ) 
    !
...
!$omp end parallel
    ...
    IF (upf(i%t)%has_so.and.nspin_mag==4.and.i%ae==1) THEN
       CALL PAW_rad2lm(i, g_rad, g_lm, i%l, nspin_mag)
       DEALLOCATE( msmall_rad )
    END IF
