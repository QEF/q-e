...
#include "machine.h"
  ...
  ! do-loop counters
  real(kind=DP), pointer :: hr (:,:),  sr (:,:), vr (:,:),  ew (:)  
  ! Hamiltonian on the reduced basis
  ! S matrix on the reduced basis
  ! eigenvectors of the Hamiltonian
  ! eigenvalues of the reduced hamiltonian
  real(kind=DP) :: DDOT
  complex(kind=DP), pointer :: psi(:,:), hpsi (:,:),spsi (:,:) 
  ...
  call mallocate( psi, ndmx, nvecx)  
  call mallocate(hpsi, ndmx, nvecx)  
  call mallocate(spsi, ndmx, nvecx)  
  call mallocate(sr, nvecx, nvecx)  
  call mallocate(hr, nvecx, nvecx)  
  call mallocate(vr, nvecx, nvecx)  
  call mallocate(ew, nvecx)  
