...
{\tt Modules} or by the {\tt PW/src} directories. Only a brief comment
...
Modules that contain the new variables used by the \phx:
...
variables are allocated also by {\tt phq\_readin} or {\tt ph\_restart}.
...
Starting point and main programs. The directory {\tt PHonon/PH} contains five 
executables whose main program are:
...
Reading input, pseudopotentials and pw punch file:
...
Routines that select the small group of q and other symmetry matrices of
the ph.x code.
...
the q point mesh and all the preparatory stuff that is needed by the
phonon code:
...
High level drivers to make the actual calculation:
...
phq_setup.f90 Setup all the simple quantities need by of the phonon. The
...
close_phq.f90 Close the above files if open.                  
\end{verbatim}

Drivers of \pwx\ to compute the band structure:

\begin{verbatim}
run_nscf.f90 This routine run pw.x to calculate the bands. It calls
...
that are used in the rest of the code. Mainly for the US/PAW part. These
routines are called by {\tt phq\_init}:
...
adddvscf.f90    Add the additional US/PAW contribution to the right-hand
...
pcgreen.f90      Orthogonalizes and solve the linear system. Used by 
                 solve_e2 and solve_e_nscf instead of the more standard method.
                 Calls cgsolve_all for doing the actual calculation.
...
Routines to calculate the derivative of the xc potential.
Note that some of them are also in Module/funct.f90:
...
dgradcorr.f90   Change of the GGA part of the xc functional.          
compute_vsgga.f90  Additional GGA term present in the noncollinear/spin-orbit
                 case.
\end{verbatim}

Routines to deal with the NLCC
...
addcore.f90    Change of the core charge for a phonon perturbation.            
               Used by dv_of_drho and add_nlcc.
dynmatcc.f90   NLCC contribution independent from the perturbed 
               wavefunctions. Called by dynmat0.
addnlcc.f90    The nlcc part of the dynamical matrix that depends on the
               perturbed potential. Called by solve_linter.
\end{verbatim}

Routines that calculate the required tensors or quantities: 
...
\begin{verbatim}
dielec.f90    Computes the dielectric tensor given the dpsi.          
\end{verbatim}

Compute the Born effective charges:
...
drhodvloc.f90    As drhodvnl for the local potential.           
...
dynmatrix.f90    Is a driver that collects the dynamical matrix, checks if
                 ...
                 equivalent q and diagonalized the matrix. The same is
                 done for zue if required from input.                 
\end{verbatim}
...
elphon.f90   This is a driver that in the case trans=.false. reads the
             induced self-consistent potential and recalculates the
             electron phonon matrix elements. It reads also the 
             dynamical matrix and rediagonalizes it.
    (readmat) reads the dynamical matrix.
    (elphel) computes the electron-phonon matrix elements.                 
    (elphsum) makes a sum over the BZ of the square moduli of the 
              el-ph matrix elements and computes phonon linewidths. This
              routine makes also linear interpolation on k points 
...
write_epsilon_and_zeu.f90 Use summarizes, but contains also old 
                 instructions to write the dielectric constant and 
                 the born effective charges in the dynamical matrix file.

write_modes.f90  This routine writes the modes on output.
...
Routines that write on file the induced charge density:
...
Routines to read and write the xml files with the partial results:
...
Routines that read and write the recover file:

\begin{verbatim}
phq_recover.f90 This routine reads the recover files and reconstruct the
                status of the calculation so far.
write_rec.f90   This file contains the routines to read and write the 
...
and corresponding parallel routines that collect on a single processor 
the quantity to symmetrize and then call the previous routines:
...
Routines that perform the symmetry analysis of the eigenvectors to decide
...
Routines that are needed to apply the {\tt gamma\_gamma} trick:
...
Obsolete routines that are there for compatibility with other codes that
might use them:
