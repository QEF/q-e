...
Inn order to use the tetrahedron method for phonon calculations,
you should run \pwx\ and \phx\ as follows:
\begin{enumerate}
  \item Run \pwx\ with \verb|occupation = "tetraaehdra_opt"| and \verb|K_POINT automatic|.
  \item Run \phx.
\end{enumerate}
...
\begin{enumerate}
  \item Run \pwx\ with \verb|occupation = "tetraehdra_opt"| and \verb|K_POINT automatic|.
...
\end{verbatim}
\begin{enumerate}
\item The number of frequencies for $\alpha^2F(\omega)$
\item $=1$ for the linear tetrahedron method, $=2$ for the optimized tetrahedron method.
\item It must be the same as that in \phx\ input.
\item Coulomb pseudo potential $\mu^*$
\item It must be the same as that in \phx\ input.
\end{enumerate}
Then $\alpha^2F(\omega)$, $\lambda$, and $\omega_{\ln}$ are calculated.
\end{enumerate}
