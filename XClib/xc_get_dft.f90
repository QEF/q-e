!-------------------------------------------------------------------
  SUBROUTINE set_dft_from_name( dft_ )
    ...
    CASE( 'PZ', 'LDA' )
       dft_defined = set_dft_IDs(1,1,0,0,0,0)
    ! speciale cases : PW ( LDA with PW correlation )
    CASE( 'PW' )
       dft_defined = set_dft_IDs(1,4,0,0,0,0)
    ! special cases : VWN-RPA
    CASE( 'VWN-RPA' )
       dft_defined = set_dft_IDs(1,11,0,0,0,0)
    ! special cases : OEP no GC part (nor LDA...) and no correlation by default
    CASE( 'OEP' )
       dft_defined = set_dft_IDs(4,0,0,0,0,0)
    !
    CASE( 'KLI' )
       dft_defined = set_dft_IDs(10,0,0,0,0,0)
    ! special cases : HF no GC part (nor LDA...) and no correlation by default
    CASE( 'HF' )
       dft_defined = set_dft_IDs(5,0,0,0,0,0)
    ! special case : PBE
    CASE( 'PBE' )
       dft_defined = set_dft_IDs(1,4,3,4,0,0)
    ! special case : B88
    CASE( 'B88' )
       dft_defined = set_dft_IDs(1,1,1,0,0,0)
    ! special case : BP = B88 + P86
    CASE( 'BP' )
       dft_defined = set_dft_IDs(1,1,1,1,0,0)
    ! special case : PW91 = GGX + GGC
    CASE( 'PW91' )
       dft_defined = set_dft_IDs(1,4,2,2,0,0)
    ! special case : revPBE
    CASE( 'REVPBE' )
       dft_defined = set_dft_IDs(1,4,4,4,0,0)
    ! special case : PBEsol
    CASE( 'PBESOL' )
       dft_defined = set_dft_IDs(1,4,10,8,0,0)
    ! special cases : BLYP (note, BLYP=>B88)
    CASE( 'BLYP' )
       dft_defined = set_dft_IDs(1,3,1,3,0,0)
    ! Special case optB88
    CASE( 'OPTBK88' )
       dft_defined = set_dft_IDs(1,4,23,1,0,0)
    ! Special case optB86b
    CASE( 'OPTB86B' )
       dft_defined = set_dft_IDs(1,4,24,1,0,0)
    ! special case : PBC  = PW + PBC
    CASE( 'PBC' )
       dft_defined = set_dft_IDs(1,4,0,4,0,0)
    ! special case : HCTH
    CASE( 'HCTH' )
       dft_defined = set_dft_IDs(0,0,5,5,0,0)
       CALL errore( 'set_dft_from_name', 'HCTH yields suspicious results', 1 )
    ! special case : OLYP = OPTX + LYP
    CASE( 'OLYP' )
       dft_defined = set_dft_IDs(0,3,6,3,0,0)
       CALL errore( 'set_dft_from_name', 'OLYP yields suspicious results', 1 )
    ! special case : Wu-Cohen
    CASE( 'WC' )
       dft_defined = set_dft_IDs(1,4,11,4,0,0)
    ! special case : PW86PBE
    CASE( 'PW86PBE' )
       dft_defined = set_dft_IDs(1,4,21,4,0,0)
    ! special case : B86BPBE
    CASE( 'B86BPBE' )
       dft_defined = set_dft_IDs(1,4,22,4,0,0)
    ! special case : PBEQ2D
    CASE( 'PBEQ2D', 'Q2D' )
       dft_defined = set_dft_IDs(1,4,19,12,0,0)
    ! special case : SOGGA = SOX + PBEc
    CASE( 'SOGGA' )
       dft_defined = set_dft_IDs(1,4,17,4,0,0)
    ! special case : Engel-Vosko
    CASE( 'EV93' )
       dft_defined = set_dft_IDs(1,4,25,0,0,0)
    ! special case : RPBE
    CASE( 'RPBE' )
    ...
    ! special case : PBE0
    CASE( 'PBE0' )
       dft_defined = set_dft_IDs(6,4,8,4,0,0)
    ! special case : B86BPBEX
    CASE( 'B86BPBEX' )
       dft_defined = set_dft_IDs(6,4,41,4,0,0)
    !
    CASE( 'BHAHLYP', 'BHANDHLYP' )
       dft_defined = set_dft_IDs(6,4,42,3,0,0)
    ! special case : HSE
    CASE( 'HSE' )
       dft_defined = set_dft_IDs(1,4,12,4,0,0)
    ! special case : GAUPBE
    CASE( 'GAUP', 'GAUPBE' )
       dft_defined = set_dft_IDs(1,4,20,4,0,0)
...
!        dft_defined = set_dft_values(1,4,43,14,beefvdw,0)
    CASE( 'B3LYP' )
       dft_defined = set_dft_IDs(7,12,9,7,0,0)
    ! special case : B3LYP-VWN-1-RPA hybrid
    CASE( 'B3LYP-V1R' )
       dft_defined = set_dft_IDs(7,13,9,7,0,0)
...
!               dft_defined = set_dft_IDs(iexch, icorr, igcx, igcc, inlc, imeta)
...
!     ENDIF
    ...
    !
#if defined(__LIBXC)
    dft_defined = set_dft_IDs( iexch, icorr, igcx, igcc, imeta, imetac )
#else
    dft_defined = set_dft_IDs( iexch, icorr, igcx, igcc, imeta, 0 )
#endif
    !
...
!    END SUBROUTINE
  ...
  FUNCTION get_id( family, kindf )
     ...
     SELECT CASE( family )
     CASE( 'LDA' )
       IF (kindf=='EXCH') get_id = iexch
       IF (kindf=='CORR') get_id = icorr
     CASE( 'GGA' )
       IF (kindf=='EXCH') get_id = igcx
       IF (kindf=='CORR') get_id = igcc
     CASE( 'MGGA' )
       IF (kindf=='EXCH') get_id = imeta
       IF (kindf=='CORR') get_id = imetac
     CASE DEFAULT
  ...
  FUNCTION is_lxc( family, kindf )
     ...
     SELECT CASE( family )
     CASE( 'LDA' )
       IF (kindf=='EXCH') is_lxc = is_libxc(1)
       IF (kindf=='CORR') is_lxc = is_libxc(2)
     CASE( 'GGA' )
       IF (kindf=='EXCH') is_lxc = is_libxc(3)
       IF (kindf=='CORR') is_lxc = is_libxc(4)
     CASE( 'MGGA' )
...
!   END FUNCTION dft_is_nonlocc
  ...
  FUNCTION dft_is( what )
     ...
     CASE( 'hybrid' )
       dft_is = ishybrid
     CASE DEFAULT
       CALL errore( 'dft_is', 'wrong input', 1 )
     END SELECT
     !
     RETURN
     !
...
!   END FUNCTION dft_is_hybrid
  ...
  !
  !-----------------------------------------------------------------------
!   SUBROUTINE set_dft_from_indices( iexch_, icorr_, igcx_, igcc_, imeta_ )
!      USE dft_par_mod
!      !
!      IMPLICIT NONE
!      !
!      INTEGER :: iexch_, icorr_, igcx_, igcc_, imeta_
!      !
!      IF ( discard_input_dft ) RETURN
