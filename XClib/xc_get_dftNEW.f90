!-------------------------------------------------------------------
  SUBROUTINE set_dft_from_name( dft_ )
    ...
    !
#if defined(__LIBXC)
    CALL xclib_get_IDs( iexch, icorr, igcx, igcc, imeta, imetac, is_libxc )  !sistema.
#else
  ...
  SUBROUTINE set_auxiliary_flags
    ...
    !
    CALL get_exx_fraction_l( exx_fraction )
    !
    RETURN
    !
...
!   END FUNCTION dft_is_hybrid
  ...
  SUBROUTINE set_finite_size_volume( volume )
     ...
     CALL get_finite_size_cell_l( finite_size_cell_volume )
