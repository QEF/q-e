...
# Tennessee Research Foundation. All rights reserved. @copyright (c) 2012-2015
# Inria. All rights reserved. @copyright (c) 2012-2014 Bordeaux INP, CNRS (LaBRI
# UMR 5800), Inria, Univ. Bordeaux. All rights reserved.
#
# * Find FFTW Version 3 include dirs and libraries Default configuration will
#   find the real double precision fftw library version without THREADS|OMP. Use
#   this module by invoking find_package with the form: find_package(FFTW
#   [REQUIRED] # Fail with error if fftw is not found [COMPONENTS MKL]
#
# COMPONENTS can be some of the following: - MKL:     to detect the FFTW from
# Intel MKL - ESSL:    to detect the FFTW from IBM ESSL - THREADS: to detect the
# Threads version of FFTW - OMP:     to detect the OpenMP version of FFTW -
# SIMPLE:  to detect the FFTW simple precision fftw3f - LONG:    to detect the
# FFTW long double precision fftw3l - QUAD:    to detect the FFTW quadruple
# precision fftw3q
#
# This module finds headers and fftw library. Results are reported in variables:
...
# FFTW_INCLUDE_DIRS     - fftw include directories FFTW_LIBRARY_DIRS     - Link
# directories for fftw libraries FFTW_LIBRARIES        - fftw component
# libraries to be linked FFTW_INCLUDE_DIRS_DEP - fftw + dependencies include
# directories FFTW_LIBRARY_DIRS_DEP - fftw + dependencies link directories
...
# FFTW_DIR             - Where to find the base directory of fftw FFTW_INCDIR -
# Where to find the header files FFTW_LIBDIR          - Where to find the
# library files The module can also look for the following environment variables
# if paths are not given as cmake variable: FFTW_DIR, FFTW_INCDIR, FFTW_LIBDIR
# For MKL case and if no paths are given as hints, we will try to use the
...
if(NOT FFTW_FOUND)
  set(FFTW_DIR
      ""
      CACHE PATH "Installation directory of FFTW library given by user")
...
if(FFTW_DIR
   OR (FFTW_INCDIR AND FFTW_LIBDIR)
   OR ENV_FFTW_DIR
   OR (ENV_FFTW_INCDIR AND ENV_FFTW_LIBDIR))
  set(FFTW_GIVEN_BY_USER "TRUE")
endif()

# Optionally use pkg-config to detect include/library dirs (if pkg-config is
# available)
# ------------------------------------------------------------------------------
# -------
...
if((NOT PKG_CONFIG_EXECUTABLE)
   OR (PKG_CONFIG_EXECUTABLE AND NOT FFTW_FOUND)
   OR FFTW_GIVEN_BY_USER
   OR FFTW_LOOK_FOR_MKL
   OR FFTW_LOOK_FOR_ESSL)
  ...
  set(ENV_FFTW_DIR "$ENV{FFTW_HOME}")
  set(ENV_FFTW_INCDIR "$ENV{FFTW_INCLUDE}")
  if(ENV_FFTW_INCDIR)
  ...
  if(FFTW_INCDIR)
    set(FFTW_${FFTW3_HEADER_TO_FIND}_DIRS
        "FFTW_${FFTW3_HEADER_TO_FIND}_DIRS-NOTFOUND")
    find_path(
      FFTW_${FFTW3_HEADER_TO_FIND}_DIRS
  ...
  if(NOT FFTW_LOOK_FOR_MKL)
    ...
    foreach(fftw_lib ${FFTW_libs_to_find})
      ...
      if(FFTW_${fftw_lib}_LIBRARY)
        get_filename_component(${fftw_lib}_lib_path
                               "${FFTW_${fftw_lib}_LIBRARY}" PATH)
        # set cmake variables
    ...
    if(FFTW_LOOK_FOR_THREADS)
      if(FFTW_LOOK_FOR_MKL)
        if(BLAS_PAR_LIBRARIES)
          ...
          if(NOT FFTW_FIND_QUIETLY)
            message(
              STATUS "Multithreaded FFTW has been found: ${FFTW_LIBRARIES}")
          endif()
        else()
          if(NOT FFTW_FIND_QUIETLY)
            if(FFTW_FIND_REQUIRED AND FFTW_FIND_REQUIRED_MKL)
  ...
  if(FFTW_LIBRARIES)
    ...
    if(FFTW_LOOK_FOR_OMP)
      ...
      # if (CMAKE_C_COMPILER_ID STREQUAL "GNU") # either gomp ... list(APPEND
      # REQUIRED_LIBS "-lgomp") # or iomp5 list(APPEND REQUIRED_LIBS "-liomp5")
      # elseif (CMAKE_C_COMPILER_ID STREQUAL "Intel") list(APPEND REQUIRED_LIBS
      # "-liomp5") endif()
    endif()
    # MKL
    if(FFTW_LOOK_FOR_MKL)
    ...
    if(FFTW_WORKS)
      ...
      if(NOT FFTW_FIND_QUIETLY)
        message(
          STATUS
            "Looking for FFTW : test of ${FFTW_PREC_TESTFUNC}fftw_execute_ with fftw library fails"
        )
...
if(FFTW_LIBRARIES)
  ...
  if(${first_lib_path} MATCHES "(/lib(32|64)?$)|(/lib/intel64$|/lib/ia32$)")
    ...
    set(FFTW_DIR_FOUND
        "${not_cached_dir}"
        CACHE PATH "Installation directory of FFTW library" FORCE)
  else()
    set(FFTW_DIR_FOUND
        "${first_lib_path}"
        CACHE PATH "Installation directory of FFTW library" FORCE)
...
if((NOT PKG_CONFIG_EXECUTABLE)
  ...
  find_package_handle_standard_args(FFTW DEFAULT_MSG FFTW_LIBRARIES
                                    FFTW_INCLUDE_DIRS FFTW_WORKS)
else()
  find_package_handle_standard_args(FFTW DEFAULT_MSG FFTW_LIBRARIES FFTW_WORKS)
endif()
